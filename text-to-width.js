 ...
 */
	...
	//handy polyfills and utility functions
        ...
        // FIXME: NodeList can be iterated with for (item of nodelist)
	...
	if (window.NodeList && !NodeList.prototype.forEach) {
        ...
        // NOTE: because there's now Object.entries
	...
	if (!Object.forEach) {
        ...
        // FIXME: I don't even like the implementatio of the next class manipulation below.
        // it should actually at least use ClassList instead of doing it with splits etc.
        // original comment:
	...
	//synthetic events
        ...
        // FIXME, WTF
        /* became superfluous as HTMLElement.prototype.trigger exists no more.
	...
	if (!Element.prototype.matches) {
        ...
        // standard, MDN example, we don't need it anymore though, since
        // IE support can be dropped.
	...
	// not in the spec, but seems weird to be able to do it on elements but not text nodes
        // also, I don't quite get the problem, as parenNode will
        // be an Element in overwhelming contexts, otherwise the usage feels
        // disoriented.
...
*/
	...
	// shortcuts to get dimensions of element minus padding, equivalent to jQuery width() and height()
        // made into a proper function from a prototype pollution style
        function contentWidth(element) {
	        var fullwidth = element.getBoundingClientRect().width;
...
*/
		...
		TTW.updateElement = function(el) {
			...
			trigger(el, 'TextToWidth:populated');
		};
		...
		TTW.updateDelayed = function() {
			...
			if(ttwResizeTimeout) clearTimeout(ttwResizeTimeout);
			...
			ttwElements.forEach(function(el) {
				...
				if (fontkey in ttwFontElements) {
					...
					status("ffo start", fontkey);
                                        // FIXME: this is an interestig
                                        // dependency, no sure we're going
                                        // to need/keep it.
                                        console.log('new FontFaceObserver');
...
z(p,function(a){h=a;u()});t(p,L(c,'"'+c.family+'",monospace'))})})};"object"===typeof module?module.exports=A:(window.FontFaceObserver=A,window.FontFaceObserver.prototype.load=A.prototype.load);}());
	...
	export function textToWidth (sets) {
