      ...
      , CoherenceFunction.create(['font', 'fontSize', 'autoOPSZ', 'axisLocations'],
        function sanitizeAxes({font, fontSize, autoOPSZ, axisLocations}) {
            ...
            for(const [key, entry] of axisLocations) {
                ...
                const {min, max, 'default':defaultVal} = axisRanges[key];
                if(entry.value === defaultVal) {
                    // I'd prefer a flag that says explicitly whether
                    // to use default explicitly or not.
                    // This approach is about lean data, theoretically,
                    // naively, not setting the axis will apply the default,
                    // however, there are many factors that would make
                    // setting default axis values explicitly favorable.
                    // Browser bugs, overiding css defaults, ...
                    axisRanges.delete(key);
                    continue;
                }
                // And we can make sure existing axes are within the
                // min/max limits.
