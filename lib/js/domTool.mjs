...
export default class DOMTool {
  ...
  // inserts after the comment, which is not when inserting subsequent
  // elements, because the order in which they appear, the first element
  // inserted will be the last in the order of inserted elements.
  // for reuse of a marker, insertBeforeMarkerComment is probably
  // more interesting.
  ...
  static insertBeforeMarkerComment(element, marker, child, fallbackPosition) {
      var found = DOMTool.getMarkerComment(element, marker);
      if(found)
          DOMTool.insert(found, 'before', child);
      else if (fallbackPosition !== false)
          // undefined defaults to append
          DOMTool.insert(element, fallbackPosition || 'append', child);
      else
...
DOMTool.prototype.insertBeforeMarkerComment = DOMTool.insertBeforeMarkerComment;
