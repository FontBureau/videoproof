...
import {  normalizeRanges, mergePathRanges, getFullPathsFromRanges, normalizePathsRanges
       , serializePathRanges, deserializePathRanges
       , markupSelection, markupSelection2
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        doc.addEventListener('selectstart', (/* evt */)=>{
            let mergedPathRanges
              , {wipRanges, multiSelectionRanges, normalizedRanges} = this._getUISelection()
              ;
            // This is not available on iOS(-Safari etc.)
            if(this._multiSelectModifierKeyPressed) {
                // merge normalized wip-range(s) in multiSelectionRanges
                mergedPathRanges = mergePathRanges(...multiSelectionRanges, ...getFullPathsFromRanges(proof,
                        ...wipRanges.filter(
            ...
            markupSelection2(proof, 'temp-selection', mergedPathRanges);
            // drop wipRanges
             this._resetUISelection({wipRanges: [], multiSelectionRanges: mergedPathRanges, normalizedRanges});
        ...
        doc.addEventListener('selectionchange', (/* evt */)=>{
            ...
            const wipRanges = []
              , {multiSelectionRanges, normalizedRanges} = this._getUISelection()
              ;
        ...
        commentSetHighlight.addEventListener('click',evt=>{
            ...
            const { proof } = this._ui
              ...
              , { multiSelectionRanges, normalizedRanges:oldNormalizedRanges} = this._getUISelection()
              , currentSelection = []
              , normalizedRanges = []
              ;
            ...
            markupSelection2(proof, 'temp-selection', []);

            const normalizedCurrentSelection = normalizeRanges(proof, 'span.selection',
                        ...currentSelection.filter(
                                range=> !range.collapsed
                                && proof.contains(range.commonAncestorContainer)))
              ,  normalizedMultiSelectionRanges = normalizePathsRanges(proof, 'span.selection', ...multiSelectionRanges)
              ;
            if(this._multiSelectModifierKeyPressed)
...
//        });
    ...
    _resetUISelection({wipRanges=[], multiSelectionRanges=[], normalizedRanges=[]}={}) {
        this._uiSelection = {
              wipRanges
            , multiSelectionRanges
    ...
    _applyTextSelectionToProof() {
        let { proof } = this._ui;
        markupSelection(proof, 'selection', this._getMergedSelectionRanges());
    }
