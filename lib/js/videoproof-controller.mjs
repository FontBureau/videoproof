...
><legend>(INSERT LABEL)</legend>
    <label>axis <select class="grid_dimension-axis">
    </select></label>
    <label>from <input type="number" size="4" class="grid_dimension-from"></label>
    <label>to <input type="number" size="4" class="grid_dimension-to"></label>
    <label>stepping by <select class="grid_dimension-stepping">
        <option value="steps">amount</option>
        <option value="size">step size</option>
    </select></label>
    <input type="number" size="2" class="grid_dimension-stepping_value">
    <span class="grid_dimension-other-stepping_output">(X)</span>
</fieldset>`;
class UIGridDimension extends _UIBase {
    ...
    _genericInputEventHandler(elementKey, event) {
        if(elementKey === 'selectAxis') {
            this.setActiveAxis( this._uiElements.selectAxis.value);
        }
        console.log(`_genericInputEventHandler ${elementKey}: ${event.type}`, this._uiElements[elementKey].value);
        this._parentAPI.stateChangeHandler();
    }
    _initTemplate(baseClass, label) {
        ...
        for(const [key, element] of Object.entries(uiElements)) {
            const eventType = element.tagName === 'INPUT'
    ...
    get value() {
        const { selectAxis, from, to, selectStepping, steppingValue } = this._uiElements;
        if(selectAxis.value === 'disabled')
            return {axisTag: selectAxis.value};

        return {
            axisTag: selectAxis.value
          , stepping: selectStepping.value
          , from: parseFloat(from.value)
          , to: parseFloat(to.value)
          , steppingValue: parseFloat(steppingValue.value)
        };
    }

    set value({ axisTag, stepping, from, to, steppingValue}) {
        this.setActiveAxis(axisTag);

        console.warn('FIXME: validate as user input!', axisTag, from, to, stepping, steppingValue);
        this._uiElements.from.value = from;
        this._uiElements.to.value = to;
        this._uiElements.steppingValue.value = steppingValue;
        this._uiElements.selectStepping.value = stepping;
    }
    ...
    setActiveAxis(axisTag) {
        for(const option of this._uiElements.selectAxis.options) {
            if(option.value !== axisTag) continue;
            if(option.disabled)
                throw new Error(`Can't set disabled axisTag "${axisTag}"`);
            else
                break;
        }
        this._uiElements.selectAxis.value = axisTag;
        ...
        if(axisTag === 'disabled')
            return;
        // set defaults, etc.
        const {min, max} = this._axesMap.get(axisTag);
        ...
        // FIXME should not change the current amount of steps
        this._uiElements.steppingValue.value = 10;
        this._uiElements.selectStepping.value = 'steps';
    }

    disable() {
        // set the disabled option
        return this.setActiveAxis('disable');
    }
}
...
class UIGridDimensionControls extends _UIBase {
    ...
    _dimensionStateHandler(dimensionKey) {
        let changedCtrl = this._dimensionControls[dimensionKey];
        ...
        for(const [dim, ctrl] of Object.entries(this._dimensionControls)) {
            ...
            const toDisable = [];
            if(changedCtrl.axisTag !== 'disable')
    ...
    _externalUiItemsDisable(...axisTags) {
        console.error('NOT IMPLEMENTED _externalUiItemsDisable:', ...axisTags);
        // for(const axisTag of axisTags){}
    }

    _externalUiItemsReset(...axisTags) {
        console.error('NOT IMPLEMENTED _externalUiItemsReset', ...axisTags);
    }

    _setAxisToCtrl(key, axisTag) {
        this._dimensionControls[key].setActiveAxis(axisTag);
    ...
    updateFontAxes() {
        ...
        this._axesMap = new Map([
            ...
            //      , "min": 8,
            //      , "max": 144,
            //      , "default": 14, // likely irrelevant, as we'll go from min to max
            //      , "step": 0.001,
          ...
          , ['font-size', {
              ...
              , 'max': 144
              , 'default': 14
              , 'step': 0.001 // for the slider UI not for grid!
              // , 'location': 8 irrelevant
            }]
        ]);
...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        this._ui = {   status, proof, keyFramesContainer, moarAxesContainer
                     ...
                     , multipleTargets: new UIMultipleTargets(
                      ...
                      , typographyContainer, fontSize, fontSizeTo, fontSizeToContainer
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , _getDefaultManualAxisLocations = (font, fontSize) => {
                ...
                for(const [axisTag, axisDict] of Object.entries(font.axisRanges)) {
                          // Having a 10 based step is best to be able
                          // to calculate e.g. random values for the axis
                          // Tried a step size that always leads to 1000
                          // steps total, but that's messy.
                          // I only use 1/1000, so far, but it could be
                          // less bigger (1/10, 1/1) for axis with big
                          // integer range.
                    const step = 0.001 //(axisDict.max - axisDict.min) / 1000
    ...
    _uiInitProofWaterfall() {
        let { proof, fontSize, fontSizeTo, manualAxisLocations, alignment
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          , _getDefaultManualAxisLocations = (font, fontSize) => {
            ...
            ;
        // TODO case for coherenceGuard: we should  guard these to always have
        //      fontSizeTo.value = Math.max(fontSize.value, fontSizeTo.value)
        fontSize.value = 8; // start low
        // fontSizeTo.value //=> ui default (144) or its current setting

        {
            const font = this.getFont(this._animationState.fontName);
            manualAxisLocations.value = _getDefaultManualAxisLocations(font, fontSize.value);
        }
        const originalAPI = initWaterfall(proof, domTool
    ...
    _uiInitProofVarToolsGrid() {
        let { proof, fontSize, fontSizeTo, manualAxisLocations, alignment
            , variationSettingsFlags, gridDimensionControls
            } = this._ui;
        const domTool = new DOMTool(proof.ownerDocument)
          , _getDefaultManualAxisLocations = (font, fontSize) => {
                  // frame value should be propagated using CSS variables,
        ...
        const originalAPI = initVarToolsGrid(proof, domTool
                          , fontSize.value, fontSizeTo.value
                          , manualAxisLocations.value, alignment.value
                          , variationSettingsFlags.value
                          , gridDimensionControls.value)
          , augmentedAPI = Object.create(originalAPI)
          ;
        augmentedAPI.update = (changedDependencyNamesSet)=>{
            console.log('Waterfall update', ...changedDependencyNamesSet);
...
const TYPESPEC_MULTIPLE_TARGETS = ['fontSize', 'fontLeading', 'alignment', 'colors'
  ...
  , PROOF_STATE_DEPENDENCIES = {
      ...
      , "WATERFALL": ['fontSize', 'fontSizeTo', 'alignment', 'colors'
            , 'variationSettingsFlags', 'manualAxisLocations'
        ]
      , "VARTOOLS_GRID": ['fontSize','alignment','colors'
            , 'variationSettingsFlags', 'manualAxisLocations'
