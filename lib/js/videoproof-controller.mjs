...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        let doc = this._mainUIElement.ownerDocument
          ...
          , pasteState = doc.getElementById('paste-state')
          ;
        ...
        shareStateLink.addEventListener('click', evt=>{
            ...
            let state = this.getStateForSerialization()
              ...
              ;
            console.log('state serialization:', serializedStateForUrl);
        });

        pasteState.addEventListener('change', ()=>{
            let stateStr = pasteState.value.trim()
              , state = this._deserializedStateFromURL(stateStr)
              ;
            this.setState(state);
        });
