...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        fontLeading.addEventListener('input', (/*e*/)=>this._stateChangeHandler('fontLeading'));
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , multipleTargets: {
                          ...
                          , uiStateFields: null
                          , _uiStateFieldsSet: null
                          , isActive: false // will we use this ever?
                          , _activeTarget: 0
                          , isSynchronisationItem: true
                    ...
                    , manualAxisLocations: {
                      ...
                      , columnWidth: {
                          ...
                          , _enableElement: this._enableElement.bind(this)
                          , _disableElement: this._disableElement.bind(this)
                          , uiEnable: function() {
                                this.inputs.map(this._enableElement);
                            }
                          , uiDisable: function() {
                                this.inputs.map(this._disableElement);
                            }
                        }
                      ...
                      , alignment: {
                          ...
                          , initialize: function(stateChangeHandler) {
                                ...
                                if(!gotChecked)
                                    // some arbitray, last resort, default
    ...
    _updateOneToManySyncs(...dependencyNames) {
        ...
        for(let synchronisation of this._activeOneToManySynchronisations)
            moreDependencies.push(...this._updateOneToManySync(
                                synchronisation, ...dependencyNames));
    ...
    _stateChangeHandlerSilent(...dependencyNames) {
        ...
        let moreDependencies = this._updateOneToManySyncs(...dependencyNames);
        for(const dependencyName of [...dependencyNames, ...moreDependencies])
    ...
    _disableElement(element) {
        if(typeof element.uiDisable === 'function') {
            ...
            if(element.isSynchronisationItem)
                this._activeOneToManySynchronisations.delete(element);
        }
    ...
    _enableElement(element) {
        if(typeof element.uiEnable === 'function') {
            element.uiEnable();
            if(element.isSynchronisationItem) {
                let name = this._uiElementToName.get(element);
