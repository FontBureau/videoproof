...
export class VideoproofController {
    ...
    constructor(contentWindow) {
        ...
        let exhaustedInterfaceError = ()=>{throw new Error('EXHAUSTED INTERFACE: remoteResources');};
    ...
    async loadGlyphGroupsFromFetchPromises(...promises) {
        console.log('loadGlyphGroupsFromFetchPromises promises', promises);
        return Promise.all(promises)
            .then(responses=>{
                    let [response ,...rest] = responses;
                    if(rest.length)
                        console.warn('SKIPPING loadGlyphGroupsFromFetchPromises:', rest.map(resp=>resp.url).join(', '));
        ...
        for(let type of byType.keys()) {
            ...
            switch(type) {
                case 'font':
                    ...
                    this._loadFontsFromFetchPromises(...byType.get('font'));
                    break;
                case 'glyphGroups':
                    this.loadGlyphGroupsFromFetchPromises(...byType.get('glyphGroups'));
                    break;
                default:
                    console.warn(`ATTEMPT TO LOAD  UNKOWN TYPE: ${type}`);
            }
            types.push(type);
            typeResults.push(promises);
        }
        Promise.all(typeResults)
