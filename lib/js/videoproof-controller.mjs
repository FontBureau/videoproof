...
class _AnimationState {
    ...
    toKeyFramesOrderAndCoordinates() {
        ...
        return this.keyFrames.map(frame=>Array.from(zip(...frame)))
...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        //        but it's not sufficient and would have to be applied
        //        at several places.
    ...
    _uiSetAnimatonState() {
        ...
        if(baseAnimationState instanceof AnimationStateKeyFrames) {
    ...
    _deserializedStateFromURL(stateStr) {
        ...
        if(proofTag in STATE_STRUCTURE) {
            for(let [i, [key, , deserialize]] of STATE_STRUCTURE[proofTag].entries()) {
                // [value, message]
                [proofState[key], proofStateMessages[key]] = deserialize(
    ...
    setState(state) {
        ...
        if(generalStateMessages)
             console.warn(generalStateMessages.join('\n'));
        if(proofStateMessages)
             console.warn(proofStateMessages.join('\n'));
        ...
        if(fontName)
            ...
            // TODO: handle fontName === undefined (FONT NOT FOUND) ...
            console.warn(messages.join('\n'));
            return;
        }
        ...
        let font = this.getFont(fontName)
          ...
          , searchLocation = keyFrameAxisOrder.map(axisTag=>
        ...
        let moarAxesLocations = generalState.axisLocations.filter(
                ([axisTag, /*location*/])=>REGISTERED_AXES_ORDERED.indexOf(axisTag) === -1)
