...
function _filterCmap (cmap, charsSet) {
    let dz = Array.from(charsSet).pop();
...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        doc.addEventListener('keydown', evt=>{
           this._ctrlKeyPressed = evt.ctrlKey;
        });
        ...
        function _getIndexStartPath(stopNode, range) {
            return [...getIndexPath(stopNode, range.startContainer), range.startOffset];
        }
        function _getIndexEndPath(stopNode, range) {
            return [...getIndexPath(stopNode, range.endContainer), range.endOffset];
        }
        ...
        function rangeSort(a, b) {
            let [startPathA, endPathA] = _getFullPathsFromRange(proof, a)
              , [startPathB, endPAthB] =  _getFullPathsFromRange(proof,b)
              ;
            let startAb = indexPathSort(startPathA, startPathB);
            if(startAb !== 0)
                return startAb;
            let endAb = indexPathSort(endPathA, endPAthB);
            if(endAb === 0) return 0;
        ...
        function _copyRange(range) {
            let Range = doc.defaultView.Range
              , newRange = new Range()
              ;
        ...
        let getPositionKey = (stopNode, node, offset) => getIndexPath(stopNode, node).join('.') + `@${offset}`;

        let rangeToLog = range=>getPositionKey(proof, range.startContainer, range.startOffset)
                            + '--' + getPositionKey(proof, range.endContainer, range.endOffset);

        function rangesForLog(ranges) {
            return [ranges.length, ... ranges.map(rangeToLog)].join(', ');
        }
        ...
        function mergeRanges(...ranges) {
            ...
            console.log('sorted ranges', rangesForLog(ranges));
            if(!ranges.length)
                return result;
            let current = null;
            for(let range of ranges) {
                if(current === null) {
                    current = _copyRange(range);
                    continue;
                }
                // don't forget that ranges are sorted, so at least the
                // range is contained in current and at most it is outside.
                let rangeStart = _getIndexStartPath(proof, range)
                  , currentEnd = _getIndexEndPath(proof, current)
                  , comparedEndStart = indexPathSort(currentEnd, rangeStart)
                  ;
                ...
                else if(comparedEndStart >= 0) {
                    // if 0
                    //      current ends where range starts, so we can just
                    // if > 0
                    //      sort a after b
                    //      currentEnd is after range start, they overlap
                    console.log('merging', rangeToLog(range), 'into', rangeToLog(current));
                    let rangeEnd = _getIndexEndPath(proof, range)
                      , compareEndEnd = indexPathSort(currentEnd, rangeEnd)
                      ;
                    if(compareEndEnd < 0) {
                        // add them together
                        current.setEnd(range.endContainer, range.endOffset);
                    }
                    console.log('DONE', rangeToLog(current));
                }
            }
            if(current)
                result.push(current);
        ...
        function _normalizeRangePath(rootElem, path) {
            ...
            let originalIndexPath = path.slice(0, -1)
              ...
              , nodesPath = originalIndexPath.reduce((accum, index)=>
                ...
                , [])
              , flattenedIndexPath = []
              , flattenedOffset = originalOffset
            ...
            for(let i=0,l=nodesPath.length;i<l;i++) {
                ...
                for(let textNodeEquivalents of consecutiveTextNodeEquivalents) {
                    ...
                    // Only the last of these is interesting.
                ...
                if(_isTextNode(node)) {
                    ...
                    if(node.previousSibling && _isTextNodeLike(node.previousSibling)) {
                        ...
                        flattenedOffset = originalOffset + textNodeOffset;
                    }
                }
                flattenedIndexPath.push(nodeIndex - nodeIndexChange);
            }
            flattenedIndexPath.push(flattenedOffset);
            console.log('flattenedIndexPath', ...flattenedIndexPath);
            return flattenedIndexPath;
        }
        ...
        function _serializeRanges(rootElem, ...ranges) {
            console.log('_serializeRanges', ranges);
        ...
        function rangesToStr(rootElem, ...ranges) {
            let result = [];
            for(let serialized of _serializeRanges(rootElem, ...ranges)){
                let [start, end] = serialized;
                result.push(`${start.join('.')}-${end.join('.')}`);
            }
            return result.join(',');
        }

        function _parseRanges(rangesStr) {
        ...
        function _rangeFromPaths(rootElem, [startPath, endPath]) {
            let startContainerPath = startPath.slice(0, -1)
              ...
              , range = rootElem.ownerDocument.defaultView.Range()
              ;
        ...
        function _rangesFromStr(rootElem, rangesStr) {
            let [rangesPaths, message] = _parseRanges(rangesStr)
              , ranges = []
              , messages = []
              ;
            // TODO: if message
            if(message) {
                console.warn('_rangesFromStr->_parseRanges:', message);
                return [null, message];
            }
            for(let rangePaths of rangesPaths) {
                let [range, subMessage] = _rangeFromPaths(rootElem, rangePaths);
                if(message)
        ...
        function _markupSelection() {
            console.error('NOT IMPLEMENTED _markupSelection');
        }

        doc.addEventListener('selectstart', (evt)=>{
            console.log(`event ${evt.type} _ctrlKeyPressed:`, this._ctrlKeyPressed);
        ...
        doc.addEventListener('selectionchange', (evt)=>{
            ...
            //console.log(`event ${evt.type}`, evt, evt.ctrlKey, this._ctrlKeyPressed);
            ...
            // together with _ctrlKeyPressed should be OK to decide whether
            ...
            // multiple selections. We may need to remove all slection
            // ranges at selectstart or try something else.
            //while(selection.rangeCount > 1)
            //    selection.removeRange(selection.getRangeAt(0));

            console.log('startKey', startKey, 'CURRENT', this._uiSelection.currentStartKey);
            if(this._uiSelection.currentStartKey && this._uiSelection.currentStartKey !== startKey) {
                // Commit the previous range.
                console.log('this._uiSelection.currentRange.collapsed', this._uiSelection.currentRange.collapsed);
                if(!this._uiSelection.currentRange.collapsed) {
                    this._uiSelection.ranges.push(this._uiSelection.currentRange);
                    console.log(`committed range #${ this._uiSelection.ranges.length } ${rangeToLog(this._uiSelection.currentRange)}`);
                    console.log('before merge', rangesForLog( this._uiSelection.ranges));
                    ...
                    // this._uiSelection.currentRange (at least in serialization).
                    // this._uiSelection.ranges = _serializeRanges(...this._uiSelection.ranges);
                    console.log('after merge', rangesToStr(proof, ...this._uiSelection.ranges));
                    ...
                    _markupSelection(proof,
                        mergeRanges(this._uiSelection.currentRange, ...this._uiSelection.ranges));
                }
            }
            // replace the current range
            this._uiSelection.currentRange = range;
            this._uiSelection.currentStartKey = startKey;
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , aniparams, selectGlyphsContainer};
    }
    ...
    _uiProofCleanElementAttributes() {
        ...
        let requireFixedLineBreaks = new Set(['grid', 'contextual']);
        if(requireFixedLineBreaks.has(selectLayout.value))
            proof.classList.add('fixed-line-breaks');
        else
            proof.classList.remove('fixed-line-breaks');
    ...
    _uiInitProofGrid() {
        ...
        initGrid(proof, charset, fixLineBreaks.bind(null, font));
    ...
    _uiInitProofContextual() {
        let { proof, selectGlyphs, showExtendedGlyphs} = this._ui
          , font = this.getFont(this._animationState.fontName)
          , getSelectedCharset = ()=>getCharsForSelectUI(this._charGroups, font, selectGlyphs.value)
          ...
          let padMode = 'kern-mixed'
            , customPad = '1233'
            ;

        // TODO: fixLineBreaks would be good if all words get the value
        //       of the biggest word, that way, everything should be
        //      aligned. Since words are here wider than in grid-view
        //      the cell skipping dosn't work well.
        return initContextual(proof, getSelectedCharset, charsForKey
                            , fixLineBreaks.bind(null, font), showExtended
...
const PROOF_STATE_DEPENDENCIES = {
      ...
      , "CONTEXTUAL": ['fontName', 'selectGlyphs', 'showExtendedGlyphs']
      , "TYPESPEC": []
    }
  ...
  , PROOF_UI_DEPENDENCIES = {
      ...
      , "CONTEXTUAL": [...PROOF_STATE_DEPENDENCIES.CONTEXTUAL, ...VIDEPROOF_UI_DEPENDENCIES]
