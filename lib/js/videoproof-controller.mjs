...
><legend>(INSERT LABEL)</legend>
    ...
    <label class="grid_dimension-stepping_value_label"
        ...
        <input type="number" size="3" class="grid_dimension-stepping_value"></label>
...
class UIGridDimension extends _UIBase {
    ...
    _getInputStepsSizeForMinMax(min, max) {
    ...
    get from() {
        return parseFloat(this._uiElements.from.value);
    }
    set from(value) {
        if(typeof value !== 'number')
            throw new Error(`VALUE ERROR ${this.constructor.name} "from" `
                         + `must be a number but is ${typeof value}`);
        console.warn('CHECK value is between min and max.');

        const inputStep = parseFloat(this._uiElements.from.step)
           // expecting inputStep to be based on 10
         , fixedDigits = Math.abs(Math.log10(inputStep)) // step="0.001" => 3
         ;
        this._uiElements.from.value = value.toFixed(fixedDigits);
        this._setSteppingOtherOutput();
    }

    get to() {
        return parseFloat(this._uiElements.to.value);
    }
    set to(value) {
        if(typeof value !== 'number')
            throw new Error(`VALUE ERROR ${this.constructor.name} "to" `
                         + `must be a number but is ${typeof value}`);
        console.warn('CHECK value is between min and max.');
        const inputStep = parseFloat(this._uiElements.to.step)
           // expecting inputStep to be based on 10
         , fixedDigits = Math.abs(Math.log10(inputStep)) // step="0.001" => 3
         ;
        this._uiElements.to.value = value.toFixed(fixedDigits);
        this._setSteppingOtherOutput();
    }
    ...
    _massageSteppingValue(stepping, value) {
        ...
        if(stepping === 'size')
            return value.toFixed(2); // step="0.01"
        return value;
    }
    ...
    set steppingValue(value) {
        ...
        if(value <= 0)
            ...
            throw new Error(`VALUE ERROR ${this.constructor.name} "steppingValue" `
                         + `must be bigger than 0 but is ${value}.`);
...
class UIGridDimensionControls extends _UIBase {
    ...
    _dimensionStateHandler(dimensionKey) {
        ...
        for(const [dim, ctrl] of Object.entries(this._dimensionControls)) {
            ...
            console.log('disableAxes toDisable:', ... toDisable);
