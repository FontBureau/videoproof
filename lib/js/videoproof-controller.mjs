...
export class VideoproofController {
    ...
    _allInitialResourcesLoaded(remoteResources/*, uiDependencies*/) {
        ...
        // RemoteResources (esp. fonts) and UI are ready.
        // I.e. all of the app is ready.
    ...
    getStateForSerialization() {
        ...
        //     show-extended
        let [proofFormatTag, proofState] = [selectLayout.value, {}]// getProofStateForSerialization TODO;
          , generalState = {
                dateTime: new Date()
    ...
    setState(state) {
        ...
        // have to be opened.
        let forPrint = (msgs)=>{
            return Object.entries(msgs).filter(([k, v])=> v!==undefined && v.length).map(([k,v])=>`  ${k}: ${v}`).join('\n');
        };
        ...
        this._animationState = animationState;
        this._uiInitProof(); // calls itself this._uiSetAnimatonState();
