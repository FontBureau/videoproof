...
export class VideoproofController {
    ...
    *_deserializeFromURLV2(stateStr) {
    ...
    _deserializedStateFromURL(stateStr) {
        const isV1=str=>{
            const maybeProoftag = str.split(';', 4).pop();
     ...
     */
    _setStateVideoproofLegacy(state) {
    ...
    _setStateVersion2(stateGenerator) {
        const {value:generalState} = stateGenerator.next();
        this._setStateVideoproofLegacy([generalState]);
    ...
    setState(state) {
        if(state?.[0] === 'v2')
            return this._setStateVersion2(state[1]);
        return this._setStateVideoproofLegacy(state);
    }
