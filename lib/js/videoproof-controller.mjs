...
class UIColorsChooser extends _UIBase {
    ...
    setDefault() {
        this.value = this._colorDefaults.map(c=>`#${c}`);
    }
...
export class VideoproofController {
    ...
    __setColorsToProof() {
        ...
        proof.parentNode.style.setProperty('--color-fg', colorForeground);
        proof.parentNode.style.setProperty('--color-bg', colorBackground);
    }
    _setColorsToProof() {
    ...
    __uiSetState() {
        ...
        if(this._uiStateChanges.has('colors'))
            // it's interesting, as this handler is no longer good
            // for all proof, the proofs, instead, should implement the
            // update handler themselves!
            this.__setColorsToProof();
    ...
    setState(state) {
        ...
        for(let i=0,l=dependencies.length;i<l;i++) {
            ...
            if(['manualAxisLocations', 'multipleTargets', 'typeSpecTemplate', 'fontLeading', 'colors'].indexOf(proofDependency) !== -1) {
    ...
    getProofStateForSerialization(proofTag) {
        ...
        for(let proofDependency of dependencies) {
            if(proofDependency === 'fontName')
                ...
                continue;
            if(['manualAxisLocations', 'multipleTargets', 'typeSpecTemplate', 'fontLeading', 'colors'
