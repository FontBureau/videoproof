...
class UIManualAxisLocations extends _UIBase {
    constructor (parentAPI, domTool, element) {
        super();
        ...
        this._randomButton = domTool.createElement('button', null, 'randomize axis');

        this._randomButton.addEventListener('click', ()=>{
            console.log('on:click UIManualAxisLocations random');
    ...
    _axisChangeHandler(axisTag) {
        const valMap = new Map(this._value);
        valMap.set(axisTag, this._axesInterfaces.get(axisTag).value);
        this._value = Array.from(valMap);
        this._updateUI(this._getCurrentFont(), 'AXIS_CHANGE');
        this._parentAPI.stateChangeHandler();
    }

    _buildAxesInputs(font, frameMap) {

        // FIXME: This is not right!
        // That's also a deep copy, thus changes can be made to _value
        ...
        const insertElement = element=>this.element.append(element)
          ...
          , values = []
          ;
        for(const [axisTag, {name, min, max, 'default': def}]
                                    of Object.entries(font.axisRanges)) {
            // Let all axis be controlled by 1000 steps, not sure if this is a good idea.
            const value = frameMap.has(axisTag) ? frameMap.get(axisTag) : def
              , step = (max - min) / 1000
              , input = new UINummberAndRangeInput(
                    {
              ...
              ;
            values.push([axisTag, value]);
            inputs.set(axisTag, input);
        }
    ...
    _setFont(font, frame, via) {
        ...
        for(const axisUi of this._axesInterfaces.values())
            axisUi.destroy();
        [this._axesInterfaces, this._value] = this._buildAxesInputs(font, frameMap);
        // Looks like this._value is not forwarded into the proof here!
        // maybe, the initial value (from frame) must come from external,
        // i.e. UIMultipleTargets, because this is not working.
        this.element.append(this._randomButton);
    ...
    _updateUI(font, via) {
        ...
        for(const [axisTag, {'default': def}] of Object.entries(font.axisRanges)) {
            const axisUI = this._axesInterfaces.get(axisTag)
              , value = valueMap.has(axisTag)
                    ? valueMap.get(axisTag)
                    : def
              ;
            if(axisUI.value !== value)
                axisUI.value = value;
        }
    }
    ...
    onFontNameChangeCallback(fontName, frame) {
        ...
        console.log('onFontNameChangeCallback setting frame:', frame);
