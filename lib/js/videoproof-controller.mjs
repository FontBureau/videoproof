...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        let doc = this._mainUIElement.ownerDocument
          ...
          , columnWidthContainer = doc.getElementById('column-width-container')
          , columnWidthInput = doc.getElementById('column-width-input') // type: number min='10' max='80' value='40'
          , columnWidthRange = doc.getElementById('column-width-range') // type: range min='10' max='80' value='40' step='0.1'
          ...
          , fontLeading = doc.getElementById('edit-leading') // type: range min='8' max='576' value='14'

          , alignmentColorContainer = doc.getElementById('alignment-color-container')
          , alignLeft = doc.getElementById('align-left') //   type:radio name:alignment value:l
          , alignCenter = doc.getElementById('align-center') //                         value:c
          , alignRight = doc.getElementById('align-right') //                           value:r
          , alignJustify = doc.getElementById('align-justify') //                       value:j
          ;
        ...
        fontLeading.addEventListener('input', (/*e*/)=>this._stateChangeHandler('fontSize'));
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , contextualPadCustom
                      // This is only a fake element, not even a prope
                    ...
                    , multipleTargets: {
                            _states: null // this will hold the states array
                          ,uiStateFields: null
                          , isActive: false // will we use this ever?
                          , _activeTarget: 0
                          ...
                          // FIXME: we must get informed here when uiStateFields
                          // change, naturally that is in  "_stateChangeHandler" or
                          // in "__uiSetState" (rather at the beginning).
                          // _stateChangeHandler makes most sense, and it
                          // should then add "multipleTargets" to this._uiStateChanges
                          // without triggering _stateChangeHandler again ...
                          //
                          // within _stateChangeHandler getting the value
                          // form the fieldname should use the same logic
                          // as the serialize function, so we don't write
                          // this multiple times.
                          //
                          // the callback for "fontSize" would something look like:
                          //    in _stateChangeHandler
                          //    // must know what it is handling,
                          //    handler = value=>multipleTargets.setValue('fontSize', value)
                          //    // OR name could be second argument!!
                          //    handler = (value, name)=>multipleTargets.setValue(name, value)
                          //
                          // let value = this._getUIValue(changedDependency);
                          // for(let [handler, updatedDependency] of handlers[changedDependency]) {
                          //    changed = handler(value, changedDependency);
                          //    if(changed)
                          //        this._uiStateChanges.add(updatedDependency);
                          // }
                          //
                          //
                          // let value = this._getUIValue(changedDependency); // AN ARRAY!
                          // // but then a handler for multipleTargets "in"/for fontSize
                          // handler = (value, name)=> fontSize.value = multipleTargets.getValue("fontSize")
                          //
                          //
                          ...
                          , setActiveTarget(activeTarget) {
                                ...
                                // Get the values at _this._states[val];
                                const values = this.uiStateFields.map(key=>this.activeState[key]);


                                // and apply to the respective UI-elements.

                                // FIXME: maybe this should happen as the inverse
                                // of listening/handling changes in uiStateFields
                                // i.e. those fields should get updated in
                                // a handler for "multipleTargets"
                                // THAT way we wouldn't require a reference
                                // to this._ui, which we lack anyways!!!
                                for(let [key, val] of zip(this.uiStateFields, values)) {
                                    if(val === null) {
                                        // FIXME: this check and warning can be removed when it's not triggered anymore.
                                        console.warn(`multipleTargets.setActiveTarget ${key} has null value in state activeTarget ${activeTarget}.`);
                                        continue;
                                    }
                                    console.info(`multipleTargets.setActiveTarget setting ${key} to ${val}.`);
                                    this._ui[key].value = val;
                                }
                                // Not sure if the handler of "multipleTargets" should
                                // trigger this._stateChangeHandler(...this.uiStateFields);
                                // especially, because it would create a loop, but it
                                // would be good to add the fields to this._uiStateChanges
                                // as they factually changed!
                            }
                          , initialize: function(stateChangeHandler) {
                                this._stateChangeHandler = stateChangeHandler;
                            }
                          , getActiveTarget() {
                               return this._activeTarget;
                            }
                            // CAUTION: do not call this "initialize" (see below)
                          , initTargets: function(amountOfTargets, uiStateFields, defaults) {
                                console.log('multipleTargets.initTargets', amountOfTargets, 'uiStateFields:', ...uiStateFields, 'defaults:', ...defaults);
                                if(!this.isActive)
                                ...
                                for(let i=0;i<amountOfTargets;i++) {
                                    let state = Object.fromEntries(zip(uiStateFields, defaults));
                                    state.targetIndex = i;
                    ...
                    , manualAxisLocations: {
                            _state: undefined
                          , active: false
                          , element: doc.createTextNode('manualAxisLocations @#INITIAL')
                          , uiEnable: function() {
                                console.log('manualAxisLocations.uiEnable', this);
                                this._state = 'enabled';
                                this.active = true;
                                this._setFont(this._getCurrentFont(), 'ENABLE');
                                DOMTool.insertAtMarkerComment(mainUIElement, 'insert: manualAxisLocations', this.element);
                            }
                          , uiDisable: function() {
                                console.log('manualAxisLocations.uiDisable', this);
                                this._state = 'disabled';
                                this.active = false;
                                this.element.data = `manualAxisLocations @#DISABLE`;
                                DOMTool.removeNode(this.element);
                            }
                              // this is the controller
                          , getFont: this.getFont.bind(this)
                              // this is the controller
                          , getCurrentFontName: ()=>this._animationState.fontName
                          , _getCurrentFont: function() {
                                  return this.getFont(this.getCurrentFontName());
                            }
                          , _setFont: function(font, via) {
                                this.element.data = `manualAxisLocations @${font.nameVersion}#${via}`;
                            }
                          , onFontNameChangeCallback: function(fontName) {
                                let font = this.getFont(fontName);
                                this._setFont(font, 'CHANGE');
                            }
                      }
                      , columnWidthContainer
                      , columnWidth: {
                            inputs: [columnWidthInput, columnWidthRange]
                          , initialize: function(stateChangeHandler) {
                          ...
                          , get value (){ return this.inputs[0].value; }
                          , set value(val) {
                                for(let input of this.inputs)
                                    input.value = val;
                            }
                          , uiEnable: function() {
                                this.inputs.map(_enableElement);
                            }
                          , uiDisable: function() {
                                this.inputs.map(_disableElement);
                            }
                        }
                      , typographyContainer, fontSize , fontLeading
                      , alignmentColorContainer
                      // Pseudo alignment element, to centrally manage
                      // these radio inputs: alignLeft, alignCenter, alignRight, alignJustify
                      , alignment: {
                            inputs: [alignLeft, alignCenter, alignRight, alignJustify]
                          , initialize: function(stateChangeHandler) {
                                for(let input of this.inputs)
                                    input.addEventListener('change', stateChangeHandler);
                            }
                          , get value (){ return this.inputs.filter(e=>e.checked)[0].value; }
                          , set value(val) {
                                for(let input of this.inputs)
        ...
        // Remove when we switch to a class based element type with a proper
        // constructor, but this is good enough for now.
        for(let [name, item] of Object.entries(this._ui)) {
            if(!item.initialize || typeof item.initialize !== 'function')
                continue;
            item.initialize(this._stateChangeHandler.bind(this, name));
        }
    }
    ...
    __uiSetState() {
        ...
        let {status, proof, keyFramesContainer, moarAxesContainer,
               duration: uiDuration, selectFonts, selectLayout, manualAxisLocations} = this._ui
          , [frame, t, duration, /*fromKeyFrameIndex, keyFrameT, fps */] =  this._animationState.lastYield
        ...
        // FIXME: .active could be determined by PROOF_UI_DEPENDENCIES
        //        THIS also means we should maybe only change this after
        //        the proof was not freshly initialized ????
        if(manualAxisLocations.active && this._uiStateChanges.has('fontName')) {
            // could register a callback "on change of fontName" ... ?
            manualAxisLocations.onFontNameChangeCallback(this._animationState.fontName);
            // I'm yet unclear whether this could change the relevant
            // information in __uiSetState.
        }
        ...
        else if(this._ui._proofAPI.update) {
            // FIXME: should this require the updated state information???
            this._ui._proofAPI.update();
        }
    ...
    _uiInitProofTypespec() {
        let { proof, multipleTargets } = this._ui;
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , [templateElement, typoTargetAmount] = typespecTemplateToDOM(domTool, typespecTemplate)
            ...
            // FIXME: need sth. better here, maybe attached as attribute
            //       to the individual fields?
            //       ALSO in a second step, the template should define
            //       initial default values for it's targets.
          , defaults = TYPESPEC_MULTIPLE_TARGETS.map((/*name*/)=>null)
          , states = multipleTargets.initTargets(typoTargetAmount, TYPESPEC_MULTIPLE_TARGETS, defaults)
        ...
        return initTypespec(proof, domTool, templateElement, setActiveTypoTarget
    ...
    setState(state) {
        ...
        for(let i=0,l=dependencies.length;i<l;i++) {
            ...
            if(['manualAxisLocations', 'multipleTargets', 'typeSpecTemplate'].indexOf(proofDependency) !== -1) {
    ...
    getProofStateForSerialization(proofTag) {
        ...
        for(let proofDependency of dependencies) {
            if(proofDependency === 'fontName')
                ...
                continue;
            if(['manualAxisLocations', 'multipleTargets', 'typeSpecTemplate'
...
// links.
const TYPESPEC_MULTIPLE_TARGETS = ['fontSize', 'fontLeading', 'alignment'
  ...
  , PROOF_UI_DEPENDENCIES = {
      ...
      , "TYPESPEC": [...PROOF_STATE_DEPENDENCIES.TYPESPEC , 'columnWidthContainer'
