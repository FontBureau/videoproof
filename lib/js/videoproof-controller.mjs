...
export class VideoproofController {
    ...
    async initUI(mainUIElement) {
        ...
        this._mainUIElement.append(keyFramesContainer, status, axesInfo, togglePlayButton, proof);
        this._ui = {status, axesInfo, proof, keyFramesContainer};
        this._onUILoaded();
    }
    ...
    setRunning(isRunning) {
        ...
        if(!this._gen) {
            // can't init the generator, maybe there's no font or no
            // interface yet.
            // this._reportStatus('paused');
            this._running = false;
            return;
