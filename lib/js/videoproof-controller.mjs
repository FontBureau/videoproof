...
function getCharsForSelectUI(charGroups, value) {
    let chars, extendedChars;
    switch(value) {
        case 'all-gid':
        case 'all-groups':
        case 'misc':
            [chars, extendedChars] = [`FIXME: ${value}`, ''];
            break;
        default:
...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        let disableShowExtended = ()=>{
            let [, extendedChars] = getCharsForSelectUI(this._charGroups, selectGlyphs.value);
    ...
    _uiInitProof() {
        ...
        let [charset, extendedChars] = getCharsForSelectUI(this._charGroups, selectGlyphs.value);
