...
class UIManualAxisLocations extends _UIBase {
    constructor (parentAPI, domTool, element) {
        ...
        this._randomButton.addEventListener('click', (e)=>{
            ...
            for(const [axisTag, axisValue]  of this._value) {
                // if(axisTag === 'opsz')
                //     continue;
                // else
    ...
    static copyValue(value) {
        return value.map(([axisTag, axisValue])=>[axisTag, Object.assign({}, axisValue)]);
    }
    ...
    get value() {
        ...
        return _deepFreeze(this.constructor.copyValue(this._value));
    }
    set value(val) {
        if(!Array.isArray(val)) {
            ...
            this._value = this.constructor.copyValue(val);
    ...
    _axisChangeHandler(axisTag) {
        const axisValue = new Map(this._value).get(axisTag);
    ...
    _initUI() {
        ...
        for(const [axisTag, {name, min, max, step, location}] of this._value) {
            // Let all axis be controlled by 1000 steps, not sure if this is a good idea.
            const input = new UINummberAndRangeInput(
                    {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        this._ui = {   status, proof, keyFramesContainer, moarAxesContainer
                     ...
                     , multipleTargets: new UIMultipleTargets(
                          ...
                          , ['fontName']/* requiresUpdateDependencies */
                        )
    ...
    _uiInitProofTypespec() {
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , multipleTargetsGetDefaults = ()=>{
                ...
                const defaults =  typespecTemplate.defaults.map(
                    dict=>Object.assign({}, dict, {manualAxisLocations}))
                  ;
          ...
          , multipleTargetsUpdate = ( ...changedDependencyNames)=>{
                ...
                // when falsy return nothing happens
            }
