...
export class VideoproofController {
    ...
    async _allInitialResourcesLoaded(remoteResources/*, uiDependencies*/) {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        let setColorsHandler= ()=>{
            // In this case not going via _uiSetAnimationState makes
            // changing colors very smooth, even during animation.
            // Maybe, throttling the calls to _uiSetAnimationState
            // could be an option as well, to deduplicate calls to
            // _setStateLink...
            this._setColorsToProof();
            // FIXME: Not very elegant, as with comment interface below.
            if(!this._running)
                this._setStateLink();
        };
        ...
        selectGlyphs.addEventListener('change', (/*e*/)=>this.checkProof('selectGlyphs'));
        showExtendedGlyphs.addEventListener('change', (/*e*/)=>this.checkProof('showExtendedGlyphs'));
        ...
        // updating strategy.
        customText.addEventListener('input', (/*e*/)=>this.checkProof('customText'));
        ...
        reset.addEventListener('click', (evt)=>{
            ...
            [colorForeground.value, colorBackground.value] = [`#000000`, `#FFFFFF`];
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , comment, colorForeground, colorBackground
                    , colorInvertButton};
    }
    ...
    setRunning(isRunning) {
        ...
        // This is an acceptable side effect, as the state in the url is not
        // animated, and as such we should not show wrong state when running
    ...
    // but that could also be handled in this._uiSetAnimationState
    ...
    _uiSetAnimationState() {
        if(!this.appReady) {
            console.warn(`_uiSetAnimationState: UI not yet available for activating ${this._animationState.fontName}.`);
            return false;
        }
        ...
        if(this._ui.selectLayout.value !== this._uiCurrentProof){
            this._uiCurrentProof = this._ui.selectLayout.value;
            proofchanged = true;
        }
        // FIXME: do we detect font change like this?
    ...
    _uiUpdateProof() {
        ...
        // FIXME: this should be handled centrally like the dependencies description.
    ...
    _uiInitProof() {
        ...
        // this._uiSetAnimationState();
        // _uiSetAnimationState should call this function when necessary,
        // not the other way around.
        console.warn('NOT calling _uiSetAnimationState (via _uiInitProof) ...');
    }
    ...
    _uiInitProofTypeYourOwn() {
        console.log('_uiInitProofTypeYourOwn');
    ...
    getStateForSerialization() {
        ...
        let proofFormatTag = selectLayout.value
          , proofTagUC = proofFormatTag.toUpperCase()
          , proofState = this.getProofStateForSerialization(proofTagUC)
          , generalState = {
                dateTime: new Date()
    ...
    _deserializedStateFromURL(stateStr) {
        ...
        let proofTag = generalState.proofFormatTag.toUpperCase()
    ...
    setState(state) {
        ...
        let { selectLayout, colorForeground, colorBackground } = this._ui
          ...
          ;
        let dependencies = PROOF_DEPENDENCIES[proofTag]
        ...
        this._animationState = animationState;
        // doesn't call `this._setStateLink()` like this.pause() does.
        this.setRunning(false);
        ...
        this._uiSetAnimationState(); // calls itself this._uiInitProof();
        console.warn('Calling this._uiInitProof (via setState)...');
    ...
    _changeProof() {
        let { selectLayout } = this._ui
          , proofTag = selectLayout.value.toUpperCase()
          , allProofDependencies = new Set(Object.values(PROOF_DEPENDENCIES)
                .reduce((accum, item)=>{accum.push(...item);return accum;}, []))
          ;

        console.log('_changeProof proofTag', proofTag, PROOF_DEPENDENCIES[proofTag]);

        for(let proofDependency of allProofDependencies) {
            let element = this._ui[proofDependency];
            if(!element)
                continue;
            if(PROOF_DEPENDENCIES[proofTag].indexOf(proofDependency) !== -1) {
                // activate
    ...
    checkProof(dependencyName) {
        let { selectLayout } = this._ui
          , proofTag = selectLayout.value.toUpperCase()
          ;

        if(PROOF_DEPENDENCIES[proofTag].indexOf(dependencyName) === -1)
            return;
        this._uiUpdateProof();
    }
    ...
    _deserializeProofState(proofTag, items_) {
        console.log('_deserializeProofState', proofTag,  PROOF_DEPENDENCIES[proofTag]);
        let dependencies = PROOF_DEPENDENCIES[proofTag]
                    .filter(name=>name!=='fontName')
          , messages = {}, values = {}
...
function _serializeRGBColors(colors) {
   colors = colors.map(_serializeRGBColor)
    .map((c, i)=>{
        if(i === 0 && c === '000' || c === '0000000')
            // default foreground
            return '';
        if(i === 1 && c === 'FFF' || c === 'FFFFFFF')
            // default background
            return '';
        return c;
    });
...
function _deserializeRGBColor(color) {
    ...
    if(color.match(HEX_DIGITS_REGEX) === null)
        return [undefined, messageFormat(color)];
    if(color.length === 6) {
        // pass
    }
...
function _deserializeRGBColors(string) {
    ...
    if(colors[0] === '' || colors[0] === undefined)
        colors[0] = '000';
    if(colors[1] === '' || colors[1] === undefined)
        colors[0] = 'fff';
...
// The Order in here is crucially important to never change ever!
...
const PROOF_DEPENDENCIES = {
        //"_uiUpdateProof" => selectLayout/proofFormatTag
      ...
      , "TYPE-YOUR-OWN": ['customText', 'showExtendedGlyphs' /*(future: a boolean option: "avoid line breaks" that defaults to true*/]
};
