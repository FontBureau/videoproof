...
export class VideoproofController {
    ...
    __uiSetState() {
        ...
        if(this._uiStateChanges.has('proof')) {
            ...
            this._uiUpdateProofContents(); // implies this._ui._proofAPI.update()
    ...
    _uiInitProofTypespec() {
        let { proof, activeTypoTarget } = this._ui
          ;
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , states = getInitialStates(typoTargetAmount)
    ...
    getProofStateForSerialization(proofTag) {
        ...
        for(let proofDependency of dependencies) {
            ...
            if(['manualAxisLocations', 'multipleTargets'].indexOf(proofDependency) !== -1) {
                console.warn(`getProofStateForSerialization: Skipping ${proofDependency}: NOT IMPLEMENTED`);
                continue;
            }
...
const PROOF_STATE_DEPENDENCIES = {
      ...
      , "TYPESPEC": ['manualAxisLocations', 'multipleTargets' /* TODO: how to initate different typpgraphic-targets? */]
    }
