...
class UIManualAxisLocations extends _UIBase {
    constructor (parentAPI, domTool, element) {
        ...
        this._randomButton.addEventListener('click', ()=>{
            ...
            const randAxis = ({min, max})=>(max - min) * Math.random() + min;
    ...
    _initUI() {
        ...
        for(const [axisTag, {name, min, max, location}] of this._value) {
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , multipleTargetsGetDefaults = ()=>{
                ...
                for(const [axisTag, axisDict] of Object.entries(font.axisRanges)) {
                    const location = frameMap.has(axisTag)
                      ...
                      , axisValue = Object.assign({}, axisDict, {location})
                      ;
