...
class UIOTFeaturesChooser extends _UIBase {
    ...
    _makeOptions(fontFeatures) {
        ...
        this._featureInputToInfo = new Map();
        const tags = [...fontFeatures.keys()].sort();
        for(const tag of tags) {
            console.log(tag, fontFeatures.get(tag));
            ...
            const defaultSetting = uiBoolean ? 'on' : 'off'
              , frag = this._domTool.createFragmentFromHTML(
                        `<label title="${tag} (${[...feature.tables].join(', ')}): ${friendlyName}; default: ${defaultSetting}">`
                            + `<input type="checkbox" value="${tag}" ${uiBoolean ? 'checked ' : ''}/>`
                            + ` ${tag}`
                        + `</label>`)
              , input = frag.querySelector('input')
              , elem = frag.firstElementChild
              ;
            this._featureInputToInfo.set(input, [tag, feature]);
            input.addEventListener('change', e=>console.log(`changed ${e.target.value} checked: ${e.target.checked}`));
    ...
    applyFeatures(container) {
        const settings = [];
        for(const [featureInputElement, [featureTag, {info:{uiBoolean}}]
                            ] of this._featureInputToInfo.entries()) {
            const checked = featureInputElement.checked
              , isDefault = uiBoolean === checked
              ;
            if(!isDefault)
                settings.push(`"${featureTag}" ${checked ? 'on' : 'off'}`);
        }
        if(settings.length)
            container.style.setProperty('font-feature-settings', settings.join(', '));
        else
            container.style.removeProperty('font-feature-settings', settings.join(', '));
    }
