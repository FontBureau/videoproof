...
export class VideoproofController {
    ...
    _resetUISelection({currentStartKey=null, currentRange=null, normalizedRanges=[]}={}) {
        this._uiSelection = {
    ...
    _uiSelectionIsEmpty() {
        return this._uiSelection.currentStartKey === null
            && this._uiSelection.currentRange === null
            && this._uiSelection.normalizedRanges.length === 0
            ;
    }

    // For reading only! deepFreeze/deepCopy?
    _getUISelection() {
        return this._uiSelection;
    }
    ...
    _getMergedSelectionRanges() {
        let { proof } = this._ui
          , {currentRange, normalizedRanges} = this._getUISelection()
          , normalizedCurrent = this._currentRange
                    ? normalizeRanges(proof, currentRange)
                    : []
          ;

        return mergePathRanges(...normalizedRanges, ...normalizedCurrent);
    }

    _applyTextSelectionToProof() {
        let { proof } = this._ui;
        markupSelection(proof, this._getMergedSelectionRanges());
    }
