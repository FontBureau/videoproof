...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        let doc = this._mainUIElement.ownerDocument
          ...
          , shareStateLink = doc.getElementById('share-state')
          , comment = doc.querySelector('#comment-box textarea')
          ;
        ...
        shareStateLink.addEventListener('click', evt=>{
            // TODO
            evt.preventDefault();
            let state = this.getStateForSerialization()
              , serialiedStateForUrl = this._serializeStateForURL(state)
              , deserializedStateFromURL = this._deserializedStateFromURL(serialiedStateForUrl)

              ;
            this.setState(deserializedStateFromURL);
        });
    ...
    getStateForSerialization() {
        let font = this.getFont(this._animationState.fontName)
          ...
          , [frame, /*t*/, /*duration*/, /*fromKeyFrameIndex*/, /*keyFrameT*/] = this._animationState.lastYield
          ;
        ...
        //     show-extended
        let [proofFormatTag, proofState] = ['grid', {}]// getProofStateForSerialization TODO;
          , generalState = {
                dateTime: new Date()
              , fontParticles: font.serializationNameParticles
              , axisLocations: frame.map(([k, v])=>[k, Math.round(v)])
              , proofFormatTag
              , comment: this._ui.comment.value.trim()
              , highlightPlaceholder: '' // TODO
            }
          ;
        return [generalState, proofState];
    }
    _serializeStateForURL(state) {
        console.log('_serializeStateForURL', state);
        let [generalState, proofState] = state
          , structs = [STATE_STRUCTURE._GENERAL]
          , proofTag = generalState.proofFormatTag.toUpperCase()
          , data = []
          , extended = []
          ;
        if(proofTag in STATE_STRUCTURE)
            structs.push(STATE_STRUCTURE[proofTag]);

        let setExtended = (type, data)=>{
            let _serialize = STATE_STRUCTURE._EXTENDED[type][0];
            extended.push(_serialize(data), setExtended);
            return extended.length-1;
        };

        for(let [i, struct] of structs.entries()) {
            let _state = state[i]
              , serialized = []
              ;
            for(let [k, serialize, ] of struct)
                serialized.push(serialize(_state[k], setExtended));
            // remove empty '', trailing entries
            while(serialized[serialized.length-1] === '')
                serialized.pop();
            data.push(serialized.join(';'));
        }
        data.push(...extended);
        // FIXME: how to add "extended" to data AND do we need it at all?
        return data.join('&');
    }
        // TODO:
        // console.log('this._animationState', this._animationState);
        // console.log('this._animationState.baseAnimationState', this._animationState.baseAnimationState);


    // If we have *all* the axes of REGISTERED_AXES_ORDERED we can create an
    // AnimationStateKeyFrames
    // If we have one more axis, we create an AnimationStateMoar
    // if we don't have all the registered axes, we can create a
    // AnimationStateKeyFrames with default values for the other axes
    // if we have more than one unregistered axes, we won't create a
    // generator for it, instead just use one/the first unregistered and
    // drop the rest.
    //
    // if the deserialized state can't be visited with the keyframes of
    // the animations we created we are in some kind of "detached state"
    // so playing the animation from there will have to leave the detached state,
    // possibly with some kind of temporary/transitioning animation and enter
    // the standard animation at its closest point.
    // Since this is inehrently a bit unprecise, we may as well always use
    // the transition animation. The UI state that is displayed (keyframes/moar)
    // position is during that transition the position of the re-entry.
    // Despite of the "aniparams" display, which informs about the state being
    // detached and about the none-default axis that are being animated.
    ...
    //   "axisTag": "XOPQ",
    //   "keyFrames": [
    //     [["opsz",14], ["wdth",25], ["wght",259.7199999928465], ["XOPQ",96]],
    //     [["opsz",14], ["wdth",25], ["wght",259.7199999928465], ["XOPQ",27]],
    //     [["opsz",14], ["wdth",25], ["wght",259.7199999928465], ["XOPQ",175]]
    //   ],
    //   "_lastYield": [
    //     [["opsz",14], ["wdth",25], ["wght",259.7199999928465], ["XOPQ",135.16425000470878]],
    //     0.8347499999801318,
    //     6,
    //     2,
    //     0.5042499999403951,
    ...
    //       [["opsz",  8], ["wdth",  100], ["wght",  100]],
    //       [["opsz",  8], ["wdth",  100], ["wght",  1000]],
    //       [["opsz",  8], ["wdth",  25], ["wght",  400]],
    //       [["opsz",  8], ["wdth",  25], ["wght",  100]],
    //       [["opsz",  8], ["wdth",  25], ["wght",  1000]],
    //       [["opsz",  8], ["wdth",  151], ["wght",  400]],
    //       [["opsz",  8], ["wdth",  151], ["wght",  100]],
    //       [["opsz",  8], ["wdth",  151], ["wght",  1000]],
    //       [["opsz",  14], ["wdth",  100], ["wght",  400]],
    //       [["opsz",  14], ["wdth",  100], ["wght",  100]],
    //       [["opsz",  14], ["wdth",  100], ["wght",  1000]],
    //       [["opsz",  14], ["wdth",  25], ["wght",  400]],
    //       [["opsz",  14], ["wdth",  25], ["wght",  100]],
    //       [["opsz",  14], ["wdth",  25], ["wght",  1000]],
    //       [["opsz",  14], ["wdth",  151], ["wght",  400]],
    //       [["opsz",  14], ["wdth",  151], ["wght",  100]],
    //       [["opsz",  14], ["wdth",  151], ["wght",  1000]],
    //       [["opsz",  144], ["wdth",  100], ["wght",  400]],
    //       [["opsz",  144], ["wdth",  100], ["wght",  100]],
    //       [["opsz",  144], ["wdth",  100], ["wght",  1000]],
    //       [["opsz",  144], ["wdth",  25], ["wght",  400]],
    //       [["opsz",  144], ["wdth",  25], ["wght",  100]],
    //       [["opsz",  144], ["wdth",  25], ["wght",  1000]],
    //       [["opsz",  144], ["wdth",  151], ["wght",  400]],
    //       [["opsz",  144], ["wdth",  151], ["wght",  100]],
    //       [["opsz",  144], ["wdth",  151], ["wght",  1000]]
    //     ],
    //     "_lastYield": [
    //       [["opsz",  14], ["wdth",  25], ["wght",  259.7199999928465]],
    //       0.4617629629638461,
    //       54,
    //       12,
    //       0.46760000002384494,
    //       9.541984726312062
    //     ],
    //     "fontName": "from-url RobotoFlex Regular Version_2-136"
    //   }
    // }


    //    // temporary to see if it makes any sense at all ...
    //    // if doent have a baseAnimationState (Moar ) it's the base.
    //    let baseAnimationState = this._animationState.baseAnimationState || this._animationState
    //      , [frame, t, duration, fromKeyFrameIndex, keyFrameT/*, fps */] = baseAnimationState.lastYield
    //      , [, c] = zip(...frame)
    //      , keyFramesCoordinates = baseAnimationState.keyFrames.map(frame=>Array.from(zip(...frame)).pop())
    //      ;
    //    console.log('t', t, 'fromKeyFrameIndex', fromKeyFrameIndex, 'keyFrameT', keyFrameT);
    //    //  let [distance, t, point] = closestPoint(a, b, c);
    //    console.log('c', c);
    //    // Questions about rounding of axis values are discussed elsewhere.
    //    c = c.map(cn=>Math.round(cn));
    //    console.log('c rounded:', c);
    //
    //    // This is the end of serialization of the current frame.
    //    fontName
    //
    //    // console.log('keyFramesCoordinates', keyFramesCoordinates);
    //    let distances = keyFramesCoordinates.map((keyframe, i, arr)=>{
    //        let nextKeyframe = i + 1 === arr.length ? arr[0] : arr[i+1];
    //        return  /*[distance, t, point] = */ closestPoint(keyframe, nextKeyframe, c);
    //    });
    //    console.log('distances [distance, t, point]', distances.map(([d, t, p])=> [d, t, p.join(', ')]));
    //
    //    let fromKeyFrameIndexRestored = 0
    //      , lowestD = distances[fromKeyFrameIndexRestored][0]
    //      ;
    //    for(let [i, [d, ]] of distances.entries()) {
    //        if(d < lowestD) {
    //            lowestD = d;
    //            fromKeyFrameIndexRestored = i;
    //        }
    //    }
    //    console.log('fromKeyFrameIndexRestored', fromKeyFrameIndexRestored, 'lowestD', lowestD, `distances[${fromKeyFrameIndexRestored}] ...`, ...distances[fromKeyFrameIndexRestored]);
    //
    //    let [/* d */, keyFrameTRestored /*, p */] = distances[fromKeyFrameIndexRestored]
    //      , keyFramesPosition = fromKeyFrameIndexRestored + keyFrameTRestored
    //      , tRestored = keyFramesPosition/keyFramesCoordinates.length
    //      ;
    //    console.log('t', tRestored, 'keyFramesPosition', keyFramesPosition);

        // serialize as a struct:
        //
        ...
        // 4. str Comment
        // 5. ideally we can serialize the browser select API state Custom string highlight (TODO)
        //    maybe this must also live in the Proof format, if it's special how to select within the
        //    proof.
        // ~~6. Custom string input (Part of the proof format widget "type-your-own")~~

//        https://videoproof.graphicore.de/?comment=&
//        font=RobotoFlex%5BGRAD%2CXOPQ%2CXTRA%2CYOPQ%2CYTAS%2CYTDE%2CYTFI%2CYTLC%2CYTUC%2Copsz%2Cslnt%2Cwdth%2Cwght%5D&
//        mode=grid&
//        glyph-group=all-gid&
//        foreground=%23000000&
//        background=%23ffffff&
...
//
//
//        = can be used to separate key=value
//        & can be used to separate kv&kv&kv
//        but we can also use
//        : to separate key:value
//        and
//        ; to separate kv;kv;kv
//
//        we can either use some sort of brackets {}[] to encapsulate nested values
//        but parsing this is not ideal
//        on the other hand, it could serialize nicer than encodeURIComponent(JSON.stringify(dataObject))
//        which becomes rather unreadable
//
//        another way could be to put complex objects to the end of the url...
//
//        // dateTime = > str = `${d.toISOString().slice(0, -5)}Z`
//        // => '2022-05-09T21:46:47Z'
//        // we'll have to add do restore the correct iso string, but getting rid
//        // of the miliseconds saves space in the url and we don't need this kind of precision
//        // '2022-05-09T21:46:47Z'.slice(0,-1) + '.000Z'
//        // this datetime is also URL-save enough for us, we don't encode it further!
//
//        datetime;list of str font-name, version; list of axislocations atag:number;proof format tag; str comment; str highlight&proof custom state data&more structured data
//        // 0. Comment time and date.
//        // 1. Version of font (we use array of srings [fontName, version])
//        // 2. Designspace location (we custom order by axes tags in here, array of [str axisTag, Number(parseFLoat) axisLocation])
//        // 3. Proof format (sub format depending on the proof widget)
...
//        //    proof.
    ...
    _deserializedStateFromURL(stateStr) {
        console.log('_deserializedStateFromURL', stateStr);
        let [generalData, proofCustomData, ...extendedStructuredData] = stateStr.split('&')
          , generalItems = generalData.split(';')
          , proofCustomItems = proofCustomData.split(';')
          , generalState = {}
          , proofState = {}
          , getExtended = (type, pointer)=>{
                let _deserialize = STATE_STRUCTURE._EXTENDED[type][1];
                /* Should also return [value, message], but it may become
                 * a bit akward to work with this data.
                 */
                if(extendedStructuredData[pointer] === undefined)
                    return [undefined, `Entry not defined at index ${pointer}`];
                return _deserialize(extendedStructuredData[pointer], getExtended);
            }
          ;
        for(let [i, [key, , deserialize]] of STATE_STRUCTURE._GENERAL.entries()) {
                           // [value, message]
            generalState[key] = deserialize(
                        generalItems[i] === undefined ? '' : generalItems[i]
                      , getExtended
            );
        }

        let proofTag = generalState.proofFormatTag[0].toString().toUpperCase();
        if(proofTag in STATE_STRUCTURE) {
            for(let [i, [key, , deserialize]] of STATE_STRUCTURE[proofTag].entries()) {
                // [value, message]
                proofState[key] = deserialize(
                        proofCustomItems[i] === undefined ? '' :  proofCustomItems[i]
                      , getExtended
                );
            }
        }
    ...
    _getFontFromSerialization(state) {
        ...
        // and wan't to start new, e.g. because the font we have chosen
        ...
        let fontObj = this._fonts.get(fontName).fontObject
         , axisRanges = _getFontAxisRanges(fontObj)
         ;
        for(let [axisTag, axisValue] of state.axisLocations) {
            if(!(axisTag in axisRanges)) {
                messages.push('Mising Axis ${axisTag} (at value: ${axisValue})');
                continue;
            }
    ...
    setState(state) {
        let [generalState, proofState] = state;
        console.log('setState generalState:');
        for(let [k, [v, message]] of Object.entries(generalState))
            console.log(`    ${k}:`,v, `MSG: ${message}`);

        console.log('setState proofState:');
        for(let [k, [v, message]] of Object.entries(proofState))
            console.log(`    ${k}:`, v, `MSG: ${message}`);

        // FIXME: state deserialization should check for types
        ...
        let fontState = {
                fontParticles: generalState.fontParticles[0]
              , axisLocations: generalState.axisLocations[0]
            }
          , [fontName, messages/*, choices */] = this._getFontFromSerialization(fontState)
          ;

        // TODO: handle fontName === undefined ...
        console.log('_getFontFromSerialization:', fontName, 'with messages:\n   ', messages.join('\n    '));
        // now we build the AnimationStates
        // There may be an AnimationState in the request that we don't now
