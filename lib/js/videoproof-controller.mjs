...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , multipleTargets: {
                          ...
                          , setActiveTarget(val) {
                                console.log('multipleTargets.setActiveTarget to:', val, 'was:', this._activeTarget);
                                // Collect current values at _activeTarget
                                // (should rather be always up to date) and
                                // save to this._states[this._activeTarget]..
                                this._activeTarget = val;
                                // Get the values at _this._states[val];
                                const values = this._states[val];
                                // and apply to the respective UI-elements.
                                throw new Error('No values have ever been set here!');
                                for(let [key, val] of zip(this._uiStateFields, values))
                                    this._ui[key] = val;
                                this._stateChangeHandler('multipleTargets', ...this._uiStateFields);
                            }
                          , getActiveTarget() {
                               return this._activeTarget;
                            }
                          , init: function(amountOfTargets, uiStateFields, defaults) {
                                console.log('multipleTargets.init', amountOfTargets, 'uiStateFields:', ...uiStateFields);
                                if(!this.isActive)
                                ...
                                this._uiStateFields = uiStateFields;
                                // that's like a reset
                                this._states = [];
                                for(let i=0;i<amountOfTargets;i++)
                                    this._states.push({targetIndex: i});
    ...
    _uiInitProofTypespec() {
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , states = multipleTargets.init(typoTargetAmount, TYPESPEC_MULTIPLE_TARGETS)
