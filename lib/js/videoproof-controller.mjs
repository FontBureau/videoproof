...
const UI_DIALOG_FONT_EXISTS =`<dialog>
    <form method="dialog">
      <fieldset>
        ...
        <code class="dialog_font-name"></code><br />
        is already loaded.
        </p>
        <menu>
...
class UIDialogFontExists {
    ...
    _initTemplate() {
        const dialog = this._domTool.createFragmentFromHTML(UI_DIALOG_FONT_EXISTS).firstElementChild
          , name = dialog.querySelector('.dialog_font-name')
          ;
    ...
    show(font) {
        ...
        this._domTool.clear(this._name);
        this._domTool.appendChildren(this._name, [`"${font.nameVersion}"`, this._domTool.createElement('br'), `(key: ${font.fullName})`]);
...
export class VideoproofController {
    ...
    async _uiDialogHandleFontExists(font) {
        if(!this.appReady)
            // It's actually untested.
            throw new Error(`Can't run _uiDialogHandleFontExists before app is ready.`);
        // font.fullName
        // font.nameVersion
        // font.serializationNameParticles


        const domTool = new DOMTool(this._mainUIElement.ownerDocument)
          , dialog = new UIDialogFontExists(domTool)
          ;
        return dialog.show(font);
    }
    ...
    async __activateFont(fontName) {
        ...
        if(this._animationState && this._animationState.fontName !== fontName) {
            ...
            const fullName = this._animationState.fontName
              , oldActiveFont = this.getFont(fullName)
              , fontBuffer = oldActiveFont.buffer && !(oldActiveFont.origin instanceof FontOriginFile)
    ...
    *_deserializeFromURLV2(stateStr) {
        ...
        for(const dependencyName of dependencies) {
            ...
            console.warn('FAILED loading proofState:', proofState);
            throw error;
        }
    ...
    async _setStateVersion2(stateGenerator) {
        ...
        const {value: proofState} = stateGenerator.next()
          , changedDependencyNames = new Set()
          ;
