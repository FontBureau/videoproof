...
function _uiBuildGlyphsSelectOptions(doc, glyphGroups) {
    let root = doc.createDocumentFragment()
      , showgroup = (name, group, optgroup=false, extended='') => {
        ...
        let extendedData = _extended.join('');
        if(extendedData)
...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, glyphGroups}) {
        ...
        let disableShowExtended = ()=>{
            let option = selectGlyphs.querySelector(':checked');
            showExtendedGlyphs.disabled = !('extended' in option.dataset);
        };
        disableShowExtended();
    ...
    _uiInitProof() {
        ...
        let glyphset = selectGlyphs.value;
        // FIXME: it's only disabled if option has no dataset.extended,
        // hence, one of the checks is not required.
        if(!showExtendedGlyphs.disabled && showExtendedGlyphs.checked){
            // this alone makes it so inelegant to store the data directly
            // in the select interface.
            let option = selectGlyphs.querySelector(`:checked`);
            if('extended' in option.dataset)
                glyphset += option.dataset.extended;
        }
