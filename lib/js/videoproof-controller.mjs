...
function _fixLineBreaks(element) {
    ...
    let fontsizePx = sizeToSpace(element)
    ...
    for(let item of element.children) {
        ...
        if (box.width > 0) {
            ...
            if (!item.style.width) {
                ...
                item.style.setProperty('--width-factor', Math.ceil(box.width / fontsizePx));
                item.style.width = `calc(1em * var(--width-factor))`;
                ...
                item.style.marginRight = `calc(0.3em * var(--width-factor))`;
            }
        // In grid-view we don't need to build lines.
        //    // I wonder actually if these need to be fixed in lines
        //    // they should not break while animating when measured approprietly
        //    // And we can probably also set the boxes to overflow without breaking...
        //    // If so, not doing this could spare some rendering time!
        //    if (box.left < lastX) {
        //        // a line break
        //        if (line && line.length)
        //            lines.push(line);
...
function getCharsFromCharGroups(charGroups, keyPath) {
    let target = keyPath.split('.')
      ...
      , chars, extendedChars
      , getGlobalExtended = chars=>Array.from(chars)
...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        let disableShowExtended = ()=>{
            // this is annoying, as we require a font here already
            // but initially there's no option with extended chars
            // selected anyways, so we just do nothing...
            if(this._animationState === null)
                return;
            let font = this.getFont(this._animationState.fontName);
            let [, extendedChars] = getCharsForSelectUI(this._charGroups, font, selectGlyphs.value);
        ...
        // TODO: needs to handle activation of the showExtendedGlyphs checkbox as well!
        // TODO: in some layouts these should be display: None and disabled.
        //       as they have no effect.
        selectGlyphs.addEventListener('change', (/*e*/)=>{
            // TODO: If this was done in _uiInitProof, we would not have to
            // call getCharsForSelectUI twice in short sucession.
            disableShowExtended();
            this._uiInitProof();
        });
    ...
    _uiInitProof() {
        let {proof:oldProof, selectGlyphs, showExtendedGlyphs } = this._ui
