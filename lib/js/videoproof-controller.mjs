...
export class VideoproofController {
    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , activeTypoTarget: {
                            _value: 0
                          , set value(val){
                                console.log('activeTypoTarget set value to:', val, 'was:', this._value);
                                this._value = val;
                            }
                          , get value(){
                               return this._value;
                    ...
                    , multipleTargets: {
                          ...
                          , _uiStateFields: null
                          , active: false // will we use this ever?
    ...
    _uiInitProofTypespec() {
        let { proof, activeTypoTarget, multipleTargets } = this._ui;
        ...
        let getInitialStates = function(typoTargetAmount) {
            ...
            let states = [];
            for(let i=0;i<typoTargetAmount;i++)
                states.push({});
            return states;
        };
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , setActiveTypoTarget = index/* could be a key/id thing as well */=>{
                const changed = activeTypoTarget.value !== index;
                // Only trigger "on change".
                if(!changed) return;
                activeTypoTarget.value = index;
                ...
                // we don't know the old value for activeTypoTarget anymore
    ...
    setState(state) {
        ...
        for(let i=0,l=dependencies.length;i<l;i++) {
            ...
            if(['manualAxisLocations', 'multipleTargets'].indexOf(proofDependency) !== -1) {
    ...
    getProofStateForSerialization(proofTag) {
        ...
        for(let proofDependency of dependencies) {
            ...
            if(['manualAxisLocations', 'multipleTargets', 'columnWidth',
                    'typeSpecTemplate', 'fontSize', 'lineHeight', 'alignment'].indexOf(proofDependency) !== -1) {
...
// links.
const TYPESPEC_MULTIPLE_TARGETS = ['fontSize', 'lineHeight', 'alignment'
    ...
    , PROOF_STATE_DEPENDENCIES = {
      ...
      , "TYPESPEC": ['columnWidth'
                , 'typeSpecTemplate' // we don't really have a choice yet, but this prepares its
