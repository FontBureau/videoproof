    ...
    async _initUI({mainUIElement, charGroups}) {
        ...
        function rangeSort(a, b) {
            let pathRangeA = _getFullPathsFromRange(proof, a)
              ...
              ;
            return pathRangeSort(pathRangeA, pathRangeB);
        }

        function pathRangeSort(a, b) {
        ...
        function getPositionKey (stopNode, node, offset) {
            return getIndexPath(stopNode, node).join('.') + `@${offset}`;
        }

        function rangeToLog (range) {
            return getPositionKey(proof, range.startContainer, range.startOffset)
        ...
        function _pathRangesForLog(...pathRanges) {
        ...
        //       get's selected.
        function _mergePathRanges(...pathRanges) {
            let result = [];
            if(!pathRanges.length)
                return result;
            pathRanges.sort(pathRangeSort);
            console.log('mergePathRanges: sorted pathRanges', _pathRangesForLog(...pathRanges));
            let current = null;
            for(let pathRange of pathRanges) {
                ...
                if(comparedEndStart >= 0) {
                    ...
                    let  compareEndEnd = indexPathSort(currentEnd, rangeEnd);
                    if(compareEndEnd < 0) {
                        // add them together
                        current[1] = rangeEnd;
                    }
         ...
         */
        function _normalizeRanges(rootElem, ...ranges) {
        ...
        function _rangesToStr(rootElem, ...ranges) {
            return _pathRangesToStr(_normalizeRanges(rootElem, ...ranges));
        }
        ...
        function _markupSelection(rootElement, normalizedMergedPathRanges) {
        ...
        doc.addEventListener('selectionchange', (evt)=>{
            ...
            if(this._uiSelection.currentStartKey && this._uiSelection.currentStartKey !== startKey) {
                ...
                if(!this._uiSelection.currentRange.collapsed) {
                    ...
                    let normalizedCurrent =  _normalizeRanges(proof, this._uiSelection.currentRange);
                    console.log('normalizedCurrent', _pathRangesForLog(...normalizedCurrent));

                    // let normalizedPathRanges = _normalizeRanges(proof, ...this._uiSelection.ranges);
                    // console.log('after normalization', _pathRangesForLog(...normalizedPathRanges));
                    ...
                    let mergedRanges = _mergePathRanges(...this._uiSelection.ranges, ...normalizedCurrent);
                    console.log('mergedRanges', _pathRangesForLog(...mergedRanges));
                    this._uiSelection.ranges.splice(0, this._uiSelection.ranges.length, ...mergedRanges);

                    _markupSelection(proof, mergedRanges);
                }
            }
