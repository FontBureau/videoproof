...
class UIManualAxisLocations extends _UIBase {
    constructor (parentAPI, element) {
        ...
        // this represents any user input, will becomme more complex of
        // course, but the gist is that the user will change axis values
        this.element.addEventListener('click', ()=>{
            console.log('on:click UIManualAxisLocations');
            const font = this._parentAPI.getFont(this._parentAPI.getCurrentFontName())
             , valMap = new Map(this._value)
             , randAxis = ({min, max})=>(max - min) * Math.random() + min
             ;
            for(const k in font.axisRanges)
                if(k === 'opsz')
                    continue;
                else
                    valMap.set(k, randAxis(font.axisRanges[k]));

            this._value = Array.from(valMap);
            this._updateUI(this._getCurrentFont(), 'ON_CLICK');
            this._parentAPI.stateChangeHandler();
        });
    }
    _valueToString(val) {
        return val.map(([k,v])=> `${k}:${v}`).join(',');
    }
    _valueFromString(str) {
        return str.split(',').map(kv=>kv.split(':')).map(([k,vStr])=>[k, parseFloat(vStr)]);
    }
    get value() {
        return this._valueToString(this._value);
    }
    set value(valStr) {
        console.info('UIManualAxisLocations set value:', valStr);
        if(valStr === '') {
            // From the template via multipleTargets
            //
            // Should apply defaults to this._value, but I'm not sure,
            // as defaults should rather be set initially somehow.
            this._value = [];
        }
        else
            this._value = this._valueFromString(valStr);

        this._updateUI(this._getCurrentFont(), 'SET_VALUE');
    }
    uiEnable() {
        console.log('manualAxisLocations.uiEnable', this);
        this._state = 'enabled';
        this.active = true;
        this._setFont(this._getCurrentFont(), this._parentAPI.getCurrentFrame(), 'ENABLE');
    ...
    uiDisable() {
        ...
        this.active = false;
        this.element.textContent = `manualAxisLocations @#DISABLE`;
        DOMTool.removeNode(this.element);
    }
    _getCurrentFont() {
        return this._parentAPI.getFont(this._parentAPI.getCurrentFontName());
    }
    // FIXME: not sure if frame should be retrieved this way.
    // also, should maybe be called "changeFont"
    _setFont(font, frame, via) {
        const frameMap = new Map(frame)
          ;
        // That's also a deep copy, thuss changes can be made to _value
        // without side effects.
        this._value = Array.from(frameMap);
        this._updateUI(font, via);
    }
    _updateUI(font, via) {
        const valueMap = new Map(this._value)
          , axes = []
          ;
        for(let [axisTag, {name, min, max, 'default': def}] of Object.entries(font.axisRanges))
            axes.push(`>>> ${axisTag}
${name} min:${min} max:${max} default:${def}
current: ${valueMap.has(axisTag) ? valueMap.get(axisTag) : 'DEFAULT'}`);
        this.element.textContent = `(manualAxisLocations @${font.nameVersion}#${via}
----
${axes.join('\n')}
---`;
    }
    onFontNameChangeCallback(fontName, frame) {
        let font = this._parentAPI.getFont(fontName);
        console.log('frame', frame);
        this._setFont(font, frame, 'CHANGE');
    }
}
...
export class VideoproofController {
        ...
        this._ui = {status, proof, keyFramesContainer, moarAxesContainer
                    ...
                    , multipleTargets: {
                          ...
                          , initTargets: function(amountOfTargets, uiStateFields, defaults) {
                                ...
                                for(let i=0;i<amountOfTargets;i++) {
                                    ...
                                    for(let k of uiStateFields) {
                                        ...
                                        state[k] = defaults[i][k];
                                    }
                                    this._states.push(state);
                                }
                                console.log('multipleTargets this.states', this.states);
                                return this.states;
                            }
                          , uiEnable: function() {
                      ...
                      , manualAxisLocations: new UIManualAxisLocations(
                          ...
                          , doc.createElement('pre', 'manualAxisLocations @#INITIAL')
                        )
    ...
    __uiSetState() {
        ...
        if(manualAxisLocations.active && this._uiStateChanges.has('fontName')) {
            ...
            manualAxisLocations.onFontNameChangeCallback(this._animationState.fontName, frame);
    ...
    _uiInitProofTypespec() {
        let { proof, multipleTargets, columnWidth } = this._ui;
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , states = multipleTargets.initTargets(typoTargetAmount
                , TYPESPEC_MULTIPLE_TARGETS, typespecTemplate.defaults)
        ...
        const originalAPI = initTypespec(proof, domTool, templateElement, setActiveTypoTarget
          ...
          ;
        augmentedAPI.update = ()=>originalAPI.update(
