...
class UIManualAxisLocations extends _UIBase {
    constructor (parentAPI, domTool, element, requiresUpdateDependencies) {
        ...
        this._randomButton.addEventListener('click', (e)=>{
            e.preventDefault();
            // I'm not sure why this click event is triggered, but it happens
            // when pressing enter within a foused input field, that is part
            // of the form#controls (btw. controls shouldn't be in a form).
            console.log('on:click UIManualAxisLocations random', e);
            if(e.explicitOriginalTarget !== this._randomButton)
                return;

            const randAxis = ({step, min, max})=> {
                const newVal = (max - min) * Math.random()
                  // for 0.001 stepBase is 1000, really makes only sense
                  // for 10-based steps.
                  , stepBase = 1 / step
                  , fixedDecimals = Math.log10(stepBase) // Math.log10(1000) === 3
                  , fitToStep = newVal.toFixed(fixedDecimals)
                  ;
                 return fitToStep;
            };
            for(const [axisTag, axisValue]  of this._value.entries()) {
                if(axisTag === 'opsz' && this.opszIsAuto)
                     continue;
                else
                    axisValue.location = randAxis(axisValue);
            }
            this._updateUI('RANDOM');
    ...
    update(...changedDependencyNames) {
        if(!new Set(changedDependencyNames).has('fontSize'))
            return false;
        if(!this.opszIsAuto)
            return false;
    ...
    _setAutoOPSZ(autoOPSZ) {
        if(autoOPSZ === undefined) autoOPSZ = true;
    ...
    _initUI() {
        ...
        if(this._autoOPSZInput) {
            this._autoOPSZInput.closest('label').remove();
            this._autoOPSZInput = null;
        }

        const insertElement = element=>this.element.append(element);

        for(const [axisTag, {name, min, max, step, location, autoOPSZ}] of this._value.entries()) {
            // Let all axis be controlled by 1000 steps, not sure if this is a good idea.

            let opszContinerElement;
            if(axisTag === 'opsz') {
                opszContinerElement = this._domTool.createElementfromHTML('label', {}
                        , '<input type="checkbox" /> Mirror size changes');
                this._autoOPSZInput = opszContinerElement.querySelector('input');

                this._autoOPSZInput.checked = autoOPSZ === undefined
                            ? true
                            : autoOPSZ
                            ;
                this._autoOPSZInput.addEventListener('change', ()=>{
            ...
            const input = new UINumberAndRangeInput(
              ...
              ;
            if(opszContinerElement) {
                insertElement(opszContinerElement);
                input.passive = true;
            }
            this._axesInterfaces.set(axisTag, input);
        }
        // always last
        insertElement(this._randomButton);
    }
...
export class VideoproofController {
        ...
        this._ui = {   status, proof, keyFramesContainer, moarAxesContainer
                     ...
                     , multipleTargets: new UIMultipleTargets(
                      ...
                      , fontLeading: new UIFontLeading(
                          ...
                          , 'Leading'
                          , {
                                mode: UIFontLeading.MODE_AUTO
                              , value: null /* will initially be set by auto, if mode is manual */
                            }
                            // requiresUpdateDependencies
                            // It is interesting,as also the dormant multipleTargets
                            // will require update when columnWidth changes!
        ...
        const domTool = new DOMTool(proof.ownerDocument)
          ...
          , multipleTargetsGetDefaults = ()=>{
                ...
                const defaults = typespecTemplate.defaults
                    .map(dict=>Object.assign({}, dict, {manualAxisLocations}))
    ...
    setState(state) {
        ...
        for(let i=0,l=dependencies.length;i<l;i++) {
            ...
            if(['manualAxisLocations', 'multipleTargets', 'typeSpecTemplate', 'fontLeading'].indexOf(proofDependency) !== -1) {
    ...
    getProofStateForSerialization(proofTag) {
        ...
        for(let proofDependency of dependencies) {
            if(proofDependency === 'fontName')
                ...
                continue;
            if(['manualAxisLocations', 'multipleTargets', 'typeSpecTemplate', 'fontLeading'
