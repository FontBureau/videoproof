...
class UIOTFeaturesChooser extends _UIBase {
    constructor(parentAPI, domTool) {
        ...
        [this.container, this._childrenContainer, this._toggleInput] = this._initTemplate();
    }
    _initTemplate() {
        const fragment = this._domTool.createFragmentFromHTML(UI_OT_FEATURES_CHOOSER_TEMPLATE)
          ...
          , toggleInput = container.querySelector('.ot_features_chooser-head_label input')
          ;
        this._parentAPI.insertElement(fragment);
        ...
        return [container, childrenContainer, toggleInput];
    }
    ...
    _onFeatureChange(featureInputElement) {
        const tag = this._featureInputToTag.get(featureInputElement)
          , {info:{uiBoolean}} = this._features.get(tag)
          , isDefault = uiBoolean === featureInputElement.checked
          , wasDefault = !this._nonDefaultFeatures.has(tag)
          ;
        if(!isDefault)
            this._nonDefaultFeatures.add(tag);
        else
            this._nonDefaultFeatures.add(tag).delete(tag);

        if(isDefault !== wasDefault)
            this._parentAPI.stateChangeHandler();
    }

    _isChecked(tag, defaultChecked) {
        return this._nonDefaultFeatures.has(tag) ? !defaultChecked : defaultChecked;
    }
    ...
    _makeOptions(fontFeatures) {
        ...
        for(const tag of tags) {
            const feature = fontFeatures.get(tag)
              ...
              ,  checked = this._isChecked(tag, uiBoolean)
              ;
            if(uiBoolean === null)
            ...
            this._features.set(tag, {input: input, ...feature});
    ...
    setFont(fontObject) {
        ...
        if(this._nonDefaultFeatures.size)
            this._toggle(true);
    }

    applyFeatures(container) {
        const features = [];
        for(const featureTag of this._nonDefaultFeatures) {
            const {info:{uiBoolean}} = this._features.get(featureTag);
            const checked = this._isChecked(featureTag, uiBoolean);
    ...
    deserializeFromUrlGeneric(value) {
        const newTags = new Set(value.split('-'))
        ...
        if(changed) {
            for(const [tag, {input, info:{uiBoolean}}] of this._features.entries())
                input.checked = this._isChecked(tag, uiBoolean);
        }
        return changed;
    }
...
const TYPESPEC_MULTIPLE_TARGETS = [
            'manualAxisLocations', 'fontSize', 'fontLeading', 'alignment', 'colors'
    ]
  , PROOF_STATE_DEPENDENCIES = {
