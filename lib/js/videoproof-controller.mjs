...
export class VideoproofController {
    ...
    //
    // Does stuff that may be a much better fit for _changeProof!
    // Especially, clearing proof children and clearing
    // proof.style is rather out of reach.
    //
    // But it calls the actual init function of the proofs, so that means
    // it has to run whenever the content of a proof must update (due to
    // state changes)!
    //
    // Called by this.checkProof and this._uiInitProof.
    //
    __uiUpdateProof() {
        ...
        // called by __uiSetAnimationState
    ...
    /**
     * if a PROOF_STATE_DEPENDENCIES for proofTag changed,
     * call this._uiUpdateProof
     *
     * called by __uiSetAnimationState
     */
    checkProof(dependencyName) {
        ...
        if(PROOF_STATE_DEPENDENCIES[proofTag].indexOf(dependencyName) === -1)
            return;
        this._uiUpdateProof();
    }
