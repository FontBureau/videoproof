...
export class VideoproofController {
    ...
    async applyLatestFont() {
        ...
        for(let animationProperties of animationGenerator(this._contentWindow.performance, keyFrames)) {
            let [frame, fps, t, fromKeyFrameIndex, keyFrameT] = animationProperties
...
function* animationGenerator(performance, keyFrames) {
    let t = 0
      ...
      ;
     // NOTE
     // Reduced  performance.now() precision to 1ms (1/1000s) in Firefox.
     // Well 1000 fps is more than enough so I think we can live with that.
     // At 25 fps we still get 40 different possible states per frame,
     // at 60 fps we get 16,667. Less than one state would likely create
     // a bumpy animation.
     // BUT there's a workaround (from MDN):
     // Starting with Firefox 79, high resolution timers can be used if
     // you cross-origin isolate your document using the
     // Cross-Origin-Opener-Policy and Cross-Origin-Embedder-Policy headers:
     // Cross-Origin-Opener-Policy: same-origin
     // Cross-Origin-Embedder-Policy: require-corp

    if(!duration)
        // we may update this via yield from UI
        duration = 1000 * keyFrames.length * 2;
    ...
    while(true) {
        ...
        lastExecution = performance.now();
        yield [frame, fps, t, fromKeyFrameIndex, keyFrameT];
    }
}
