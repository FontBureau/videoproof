...
class UIGridDimensionControls extends _UIBase {
    constructor(parentAPI, domTool) {
        ...
        this._parentAPI = parentAPI;
        [this.element, ] = this._initTemplate(domTool);
        this._value = null;  // ???
        this._fontAxis = [];
    ...
    _initTemplate(domTool) {
        const element = domTool.createElement('div', {}, `Hello ${this.constructor.name}`);
    ...
    get value () {
        // value should be something else, but I use it so far to confirm
        // the wiring is in place
         return {
            axesIKnow: this._fontAxis
        };
    }
    set value(val) {
        console.error(`${this.constructor.name} NOT IMPLEMENTED set value:`, val);
    }
    ...
    updateFontAxes() {
        ...
        this._fontAxis = [['font-size', {
            ...
            //      , "autoOPSZ": true
            //    }]
            , ...this._uiItems.manualAxisLocations.value];
    ...
    receiveUIs(uiItems) {
        for(let k of Object.keys(this._uiItems)) {
            if(!(k in uiItems))
                throw new Error(`${this.constructor.name} TYPE ERROR missing ui item ${k}.`);
            this._uiItems[k] = uiItems[k];
        }
    }

    _resetUI(k) {
        // remove the disabling of inputs
        console.log('_resetUI:', k);
    }
