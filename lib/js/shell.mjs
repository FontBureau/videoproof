...
export class VideoproofController {
    ...
    _allInitialResourcesLoaded(resources) {
        ...
        const fonts = resources.get('font')
          , availableFonts = AvailableFontsModel.createPrimalDraft({})
          ;
        // When new fonts are added, the fonts dependency will have to
        // become a new object, this fits nicely into the immutable/draft
        // system so far, so we can re-use it here.
        // A coherence method of ApplicationModel will fill the
        // actual availableFonts value.
        // Probably ideally, the fonts dependency could direclty become
        // the local availableFonts key and the activeFontKey ForeignKey
        // then could directly link to it.
        // maybe we could define it somhow as an "InternalizedValue"
        // See model.mjs for a possible syntax
        availableFonts.push(...fonts.map(font=>[font.value.fullName, font/*a FontModel*/]));
        availableFonts.metamorphose({});

        this.state = ApplicationModel.createPrimalState({fonts: availableFonts});
    ...
    async _loadFont(fontBuffer, origin, contentDocument) {
        ...
        const fontState = FontModel.createPrimalState().getDraft();
        fontState.value = font;
        return fontState.metamorphose();
    }
    ...
    _initUI({charGroups}) {
        ...
        for(let path of this._ui.dependencies) {
         ...
         this._ui.update(this.state, changedMap);
    }
    ...
    changeState(...commands) {
        ...
        let state = this.state.getDraft();
        for(const [path, method, ...args] of commands)
            // We could think about applying constraints and validation
            // just once at the end here, to speed things up.
            // When everything is set up correctly in the metamodel,
            // the valid result should not differ (for un-manipulated
            // cases like a hand edited, serialized state) but maybe
            // it could speed it up to apply many commands at once.
            applyTo(state, path, method, ...args);
        state = state.metamorphose();
        if(state !== this.state) {
            ...
            for(const [status, data, pathInstance] of  compareResult) {
                ...
                if(status === LIST_NEW_ORDER) {
                    ...
                    console.warn(`NOT IMPLEMENTED update ${status}`);
            ...
            if(!changedMap.size)
                return;
            console.log('state has changed!', changedMap);
            // inform users
