...
export class VideoproofController {
    ...
    _initUI({charGroups}) {
        ...
        this._ui = new MainUIController({
          ...
          , changeState: (...args)=>this.changeState(...args)
        });
    ...
    changeState(...commands) {
        ...
        if( this._lockChangeState !== null )
            ...
            throw new Error('LOCK ERROR changeState is locked with the lock stating: ${this._lockChangeState}.');
        ...
        for(const [path, method, ...args] of commands)
            // We could think about appling constraints and validation
