...
export class VideoproofController {
    ...
    _allInitialResourcesLoaded(resources) {
        ...
        const fonts = resources.get('font')
            // The availableFonts dependency is direclty the local state
            // "availableFonts" as an InternalizedDependency.
            // When new fonts are added, the availableFonts dependency be
            // updated and must be injected into the state to update it
            // this.state.getDraft().metamorphose({availableFonts});
        ...
        this.state = ApplicationModel.createPrimalState({availableFonts});
    ...
    async _loadFont(fontBuffer, origin, contentDocument) {
        ...
        if(this.state) {
            // FIXME: This works, but since it's now managed differently
            //        in here, it's a bit of a detour. Also this creates a
            //        a race condition, since this is async and the font is
            //        added later, not immediately.
