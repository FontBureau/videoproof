...
export class VideoproofController {
    ...
    _allInitialResourcesLoaded(resources) {
        ...
        const availableLayoutsDraft = AvailableLayoutsModel.createPrimalDraft({});

        for(const [label, LayoutModel] of [
                        ['Example', ExampleLayoutModel]
                      , ['Key Moments', ExampleKeyMomentsLayoutModel]
                    ]) {
            const availableLayout = AvailableLayoutModel.createPrimalDraft({});
            availableLayout.get('typeClass').value = LayoutModel;
            availableLayout.get('label').value = label;
            console.log('LayoutModel.constructor.name', LayoutModel.name, LayoutModel);
            availableLayoutsDraft.push([LayoutModel.name, availableLayout]);
        }
    ...
    _initUI({charGroups}) {
        ...
        this._ui = new MainUIController({
          ...
          , withChangeState: this.withChangeState.bind(this)
            ...
            // on the app state.
          , getEntry: path=>{
                const entry = this.draftState
                    ? getEntry(this.draftState, path)
                    : getEntry(this.state, path)
                    ;
    ...
    _updateState(immutableOrDraft) {
        ...
        const compareResult = new StateComparison(this.state, state)
