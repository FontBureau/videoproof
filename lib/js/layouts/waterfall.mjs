...
const WATERFALL_CONTAINER_CLASS = 'waterfall-container'
  ...
  , WATERFALL_CONTAINER_SELECTOR = `.${WATERFALL_CONTAINER_CLASS}`
...
function makeLines(domTool, contentElement, fontSize, fontSizeTo, varProperties, variationSettingsFlags) {
...
function _handleEdited(domTool, event) {
    ...
    const newText = event.target.textContent
      , container = event.target.closest(WATERFALL_CONTAINER_SELECTOR)
      , lineContents = container.querySelectorAll(LINE_CONTENT_SELECTOR)
      ;
...
export function init(proofElement, domTool, fontSize, fontSizeTo
    ...
    makeLines(domTool, contentElement, fontSize, fontSizeTo, varProperties, variationSettingsFlags);
    ...
    return {
        ...
        update: (changedDependencyNamesSet, fontSize, fontSizeTo
            ...
            const varProperties = applyPropertyChanges(contentElement, manualAxisLocations,
                                    variationSettingsFlags);
            if(['fontSize', 'fontSizeTo', 'fontName', 'variationSettingsFlags'].some(k=>changedDependencyNamesSet.has(k))) {
                makeLines(domTool, contentElement, fontSize, fontSizeTo, varProperties, variationSettingsFlags);
                return;
            }
