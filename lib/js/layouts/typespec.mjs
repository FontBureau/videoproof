...
function _clearEmptyAncestorsUntil(node, stopNode) {
    ...
    while(node !== stopNode && (
        ...
        console.log('_clearEmptyAncestorsUntil deleting:', remove, 'stopNode', stopNode);
...
function _handleEditableDiv(domTool, event) {
    ...
    console.log('editableHost.children.length', editableHost.children.length, !!editableHost.parentElement);
    ...
    if(event.inputType === 'insertParagraph') {
        ...
        while(true) {
            ...
            if(!lastInsertedNode) {
                let newP = domTool.createElement('p', {style: 'background: PaleGreen;'});
                newP.append('');
    ...
    let cursor = selection.getRangeAt(0);
    console.log('cursor.startContainer', cursor.startContainer);
