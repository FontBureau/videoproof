...
function _handleFocusIn(domTool, setActiveTypoTarget, e) {
    const activeClass = `${TYPO_ELEMENT_CLASS}--active`
      , container = e.target.closest(TYPO_ELEMENT_SELECTOR)
      ...
      , activeTypoTargetIndex = allTypoTargets.indexOf(container)
      ;
    if(!container) return;
...
export function init(proofElement, domTool, templateElement, setActiveTypoTarget, activeTypoTarget, states) {
    ...
    proofElement.append(templateElement);
    {
        ...
        let initialEditableHost = proofElement.querySelectorAll(TYPO_ELEMENT_TARGET_SELECTOR)[activeTypoTarget];
        initialEditableHost.focus(); // triggers focusin/_handleFocusIn
    }

    return {
        update: (...args)=>{
            console.log('typespec !UPDATE!:', ...args);
        }
    };
