...
function _deleteRange(domTool, range) {
    const  { Node } = domTool.window
      ...
      ;
    console.log('_deleteRange => startAncestorSibling:', startAncestorSibling)

    let editableHost = (startContainer.closest
                            ? startContainer
    ...
    for(const node of _getAllElementsInBetween(startContainer, endContainer)) {
        ...
        if(isElement) {
            for(let i=nodeEndOffset-1;i>=nodeStartOfffset;i--){
                console.log('node.removeChild(node.childNodes[i]);', i, node.childNodes[i], 'of', node);
    ...
    let [noneEmptyStartAncestor, noneEmptyStartAncestorOffset] = _clearEmptyAncestorsUntil(startContainer, commonParentElement);
    ...
    if(!startAncestorSibling.childNodes.length && startAncestorSibling !== editableHost){
        startAncestorSibling.remove();
                               // -1 because we want to mark the position after
                               // the node before startAncestorSibling
        console.log('_deleteRange result commonParentElement', commonParentElement, `@${startAncestorSiblingOffset-1}`);
        return [commonParentElement, Math.max(0, startAncestorSiblingOffset-1)];
    }
    if(startContainer.parentNode) {
        // startContainer is still there!
        console.log('_deleteRange result startContainer', startContainer, `@${startOffset}`);
        return [startContainer, startOffset];
    }
    else if(noneEmptyStartAncestor && noneEmptyStartAncestor.parentNode){
        console.log('_deleteRange result noneEmptyStartAncestor', noneEmptyStartAncestor, `@${noneEmptyStartAncestorOffset}`);
        return [noneEmptyStartAncestor, noneEmptyStartAncestorOffset];
    }
    console.log('_deleteRange result startAncestorSibling', startAncestorSibling, `@${startAncestorSiblingOffset}`);
...
function _handleEditableDiv(domTool, event) {
    ...
    console.log('staticRanges:', staticRanges.length, ...staticRanges);
    if(staticRanges.length !== 1)
        console.warn(`ASSERTION FAILED: event target ranges are not 1 but ${staticRanges.length}.`);
    else
    ...
    for(let staticRange of staticRanges) {
        ...
        console.log('add range:', liveRange.toString(), liveRange);
    }
    ...
    console.log('=>before _deleteRanges: container', liveRanges[0].startContainer, 'offset', liveRanges[0].startOffset);
    let [startContainerAfterDelete, startOffsetAfterDelete] = _deleteRanges(domTool, editableHost, liveRanges)
      ...
      ;
    console.log('=>after _deleteRanges: container', startContainerAfterDelete, 'offset', startOffsetAfterDelete);
    ...
    if(event.inputType === 'insertParagraph') {
        ...
        while(true) {
            ...
            if(!lastInsertedNode) {
                let newP = domTool.createElement('p', {style: 'border: 1px solid pink;'});
                newP.append('');
                ...
                let newP = domTool.createElement('p', {style: 'border: 1px solid lime;'})
                ...
                lastInsertedNode.style.border = '4px solid lightblue';
        ...
        if(!cursorElement.closest('[contenteditable] p')) {
            ...
            newP.style.background = 'deeppink';
    ...
    // So, here's an issue with this: we must have at least an empty
    // textNodes in our <p> elements, so we can move the cursor inside
    // (and so they have a "natural" height.
