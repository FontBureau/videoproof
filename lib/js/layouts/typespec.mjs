...
!p:很久很久以前，在一个遥远的星系 "A long time ago in a galaxy far, far away", a famous sentence in Star Wars`
  , defaults: [
        {fontSize: 64, fontLeading: 'auto', alignment: 'l', manualAxisLocations: []}
      , {fontSize: 25, fontLeading: 'auto', alignment: 'l', manualAxisLocations: []}
      , {fontSize: 18, fontLeading: 'auto', alignment: 'l', manualAxisLocations: []}
      , {fontSize: 18, fontLeading: 'auto', alignment: 'l', manualAxisLocations: []}
      , {fontSize: 14, fontLeading: 'auto', alignment: 'l', manualAxisLocations: []}
    ]
});
/**
...
function _applyTargetsState(allTypoTargets, states) {
    for(let i=0, l=allTypoTargets.length;i<l;i++) {
        ...
        for(let k of Object.keys(state)) {
            if(k === 'manualAxisLocations') {
                const locations = state[k].map(([axisTag, {location, 'default': def}])=>{
                    // FIXME: this kind of control should be
                    //        explicit part of the axisValue, not
                    //        decided in here.
                    let override;
                    if(axisTag === 'opsz' && location === def) {
                        override = 'var(--font-size)';
                    }

                    return `"${axisTag}" ${override !== undefined ? override : location}`;
                });
                // FIXME: prefer to use custom properties, but this depends
                //        on the CSS setup a lot.
                target.style.setProperty('font-variation-settings', `${locations.join(', ')}`);
            }
            else
...
function _applyState(templateElement, columnWidth, allTypoTargets, states) {
    ...
    _applyTargetsState(allTypoTargets, states);
}

export function init(proofElement, domTool, templateElement, setActiveTypoTarget, activeTypoTargetIndex, states, columnWidth) {
    ...
    _activateTypoTarget(allTypoTargets, activeTypoTargetIndex);
    setActiveTypoTarget(activeTypoTargetIndex);
    _applyState(templateElement, columnWidth, allTypoTargets, states);
    return {
        // Update will run if the proof is no re-initalized anyways.
        update: (activeTypoTargetIndex, states, columnWidth
                )=>{
                    console.log('TYPESPEC proofAPI.update!');
                    return _applyState(templateElement, columnWidth, allTypoTargets, states);
                }
    };
