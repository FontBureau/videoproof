    ...
    for(let i=0, l=allTypoTargets.length;i<l;i++) {
        ...
        for(let k of Object.keys(state)) {
            if(k === 'manualAxisLocations') {
                // FIXME: prefer when the value here comes in as an array
                // of key, value pairs already, or even as an object with
                // the ability to serialize itself to a suitable format.
                const locations = state[k] !== ''
                    ? state[k].split(',').map(kv=>kv.split(':')).map(([k, v])=>`"${k}" ${v}`)
                    : ['"NULL" 00']
                    ;
                console.log('>>> locations:', locations);
                // FIXME: prefer to use custom properties, but this depends
                //        on the CSS setup a lot.
                target.style.setProperty('font-variation-settings', `"opsz" var(--font-size), ${locations.join(', ')}`);
            }
            else
