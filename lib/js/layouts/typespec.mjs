...
const template = String.raw`H1 H1:Heading One
H2 H2:Heading Two
H3 H3:Heading Three
...
function _renderHeading(domTool, {htmlTag, label, textContent/*, type, attributes*/}){
    ...
    // </div>
    return createElementAndChildren(domTool, 'div', {'class': `typespec-element typespec-element_heading typespec-element_heading-${htmlTag}`},[
        ['label', null, label || `Heading ${htmlTag}`]
                    // Could use _reduceDOMWhitespace(textContent) here.
      , [htmlTag, {'class': "typespec-element_target typespec-element_target_heading", 'contenteditable': ''}, textContent]
    ]);
}
...
function _renderText(domTool, {label, paragraphs/*, type, attributes*/}){
    ...
    return createElementAndChildren(domTool,
        'div', {'class': 'typespec-element typespec-element_text'}, [
            ['label', null, label || 'Text']
          , ['div', {'class': "typespec-element_target typespec-element_target_text", 'contenteditable': ''},
...
function _handleFocusIn(domTool, e) {
    const container = e.target.closest('.typespec-element')
      , proofElement = e.currentTarget // where the handler is attached to
      , activeClass = 'typespec-element--active'
      ;
    if(!container) return;
    for(const elem of proofElement.querySelectorAll(`.${activeClass}`)) {
        if(elem === container)
            continue;
...
function _handleClick(domTool, e) {
    ...
    const container = e.target.closest('.typespec-element');
    if(!container) return;
    const editableHost = container.querySelector('.typespec-element_target');
...
export function init(proofElement) {
    let domTool = new DOMTool(proofElement.ownerDocument)
      ...
      , eventListeners = [
          ...
          , ['focusin', _handleFocusIn.bind(null, domTool), false]
