...
export class ForeignKey {
    ...
    //      CASCADE => delete the entry that has the key, not sure this applies
    //      RESTRICT => can't delete the entry as long as it is referenced
    //      NO ACTION => doesn't apply
    //      SET NULL => only possible if not NOT_NULL
    //      SET DEFAULT => could use the first element, could come with a method ...
...
function getTopologicallySortedOrder(fields, keys, links, childrenDependencies) {
    console.log('getTopologicallySortedOrder fields', fields);
    console.log('getTopologicallySortedOrder keys', keys);
    console.log('getTopologicallySortedOrder links', links);
    console.log('getTopologicallySortedOrder childrenDependencies', childrenDependencies);

    // links depend on their link.keyName
    // keys depend on their key.targetName
    // fields depend on their field.dependencies
    const ownNames = new Set(allKeys(fields, keys, links))
        , noDepsSet = new Set() //S ← Set of all nodes with no incoming edge
        , requirementsMap = new Map()// [dependency] => {...dependants}
        , dependantsMap = new Map()
        ;
    for(const [name, entry] of allEntries(fields, keys, links, childrenDependencies)) {
        if(childrenDependencies.has(name)) {
            if(!ownNames.has())
                // These dependecies are going to get passed through
                // they have no dependencies on their own.
                noDepsSet.push(name);
            // else: These dependecies are defined by the local namespace
        }
        else if(fields.has(name)) {
            // this is a field
            console.log('getTopologicallySortedOrder', name, entry.name, entry.dependencies, entry);
            if(entry.dependencies.size === 0) {
                noDepsSet.add(name);
            }
            else {
                dependantsMap.set(name, new Set(entry.dependecies));
                for(const dependeny of entry.dependecies)
    ...
    console.log('requirementsMap', requirementsMap);
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    static has(key) { // in all of the local name space
        ...
        for(const map of [this.field, this.foreignKeys, this.links]) {
            if(map.has(key))
                return true;
        }
        return false;
    ...
    static *entries() { // => [name, instance of _BaseModel, Key or _BaseLink]
        yield* allEntries(this.field, this.foreignKeys, this.links);
    }


    static fromRawValue(dependenciesData, rawEntries) {
        console.log('fromRawValue', this, 'dependenciesData', dependenciesData, 'rawEntries', rawEntries);
        console.log('this.fields', this.fields);
        const entries = [];

        const localScope = new Map();
        // localScope should already own external dependencies!!!
        const _dependenciesFromModel= Model=>Object.fromEntries(
                iterMap(Model.dependencies, key=>[key, localScope.get(name)]));

        for(const [name, ...rawValue] of rawEntries) {
            console.log('fromRawValue name:', name);
            let value;
            if(this.foreignKeys.has(name))
                // Key must be a key in its target, i.e. target.has(key)
                // or to be more exact: new Map(rawEntries).get(this.foreignKeys.get(name).targetName).has(rawValue[0])
                ...
                const Model = this.fields.get(name)
                  ...
                  , childDependencies = _dependenciesFromModel(Model)
                  ;
        ...
        for(const [name, ...rawValue] of rawEntries) {
            if(this.foreignKeys.has(name) || this.fields.has(name))
                entries.push(name, localScope.get(name));
            else
                // This will fail in the constructor validation, but that's
                // better than getting dropped here silently.
        ...
        return new this(dependenciesData, entries);
    }
    ...
    static concreteFactory(className, ...definitions) {
        ...
        const staticHas = this.has.bind({fields, foreignKeys, links})
          ...
          , initOrder = getTopologicallySortedOrder(fields, foreignKeys, links, childrenDependencies)// jshint ignore:line
          ;
