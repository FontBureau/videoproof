...
export class ForeignKey {
    ...
    // TODO:
    // What to do if the referenced entry is deleted, SQL has CONSTRAINTs
    //  ON DELETE and ON UPDATE, but we can probably use the same for both cases
    ...
    //
    //      The useful ones of the above should be implemented as standard
    //      methods, on the ForeignKey class.
    //      Further, there can be a custom implementation, that is defined
    //      with the key definition.
    ...
    static fromRawValue(dependenciesData, rawEntriesList) {
        ...
        for(const name of this.initOrder) {
            if(this.childrenExternalDependencies.has(name)) {
                ...
                let value;
                if(rawValue.length)
                    // if this is inappropriate it will fail later
                    // e.g. key does not exist or key is null
                    value = rawValue[0];
                else {
                    // FIXME: this is probably rather for the coherence guard!
                    //        Or the key could know itself how to solve basics
                    //        like this
                    console.log('INIT FOREIGN KEY Requires strategy!', name);
                    const key = this.foreignKeys.get(name)
                    ...
                    if(key.notNull) {
                        if(keys.length)
                            ...
                            // will fail later, could also fail now,
                            // but it is already reported.
                            value = null;
                    }
                    else
                        value = null;
