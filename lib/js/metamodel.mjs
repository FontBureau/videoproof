...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    static concreteFactory(className, ...definitions) {
        ...
        const result = {[className]: class extends this {
            static fields = Object.freeze(new FreezableMap(fields)); // jshint ignore: line
            static keys = Object.freeze(new FreezableMap(keys)); // jshint ignore: line
            static links = Object.freeze(new FreezableMap(links)); // jshint ignore: line
        }};
    ...
    constructor(entries) {
        ...
        const value = new FreezableMap(entries.filter(([name,])=>this.fields.has(name)));
        {
            const missing = []
              , types = []
              ;
        ...
        {
            const _keys = new FreezableMap(entries.filter(([name/*, value*/])=>this.keys.has(name)));
            for(const [keyName, keyDefinition] of this.keys) {
    ...
    static fromRawValue(raw) {
        ...
        const entries = raw.map(([name, ...rawValue])=>{
            ...
            if(this.keys.has(name))
                ...
                value = Model.fromRawValue(...rawValue);
            }
            else
