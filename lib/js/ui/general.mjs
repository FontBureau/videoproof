    ...
    constructor(parentAPI) {
        ...
        this._widgets = [];
        {
            const FONT_SELECT_SETUP_VALUE_PATH = 'activeFontKey'
                ...
                ;
            widgetAndMeta.widget = new FontSelect({
                domTool: parentAPI.domTool
              , insertElement: insertElement.bind(null, widgetAndMeta.insertedElements, widgetAndMeta.host)
              , changeValueHandler: value=>parentAPI.changeState([FONT_SELECT_SETUP_VALUE_PATH, 'change', value])
            });
            this._widgets.push(widgetAndMeta);
        }
    }
