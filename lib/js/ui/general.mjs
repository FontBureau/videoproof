...
class KeyMomentsTimeline extends UIBase {
    ...
    static TEMPLATE = `<div class="key_moments_timeline">
        ...
        <button class="key_moments_timeline-add_moment" title="Add Moment">+</button>
</div>`;
    ...
    constructor(parentAPI) {
        super(parentAPI);
        [this.element, this.itemsContainer, this.addButton] = this.initTemplate();
    }
    ...
    initTemplate() {
        ...
        this._insertElement(element);
        return [element, itemsContainer, addButton];
    }
    _createKeyMomentButton(index, keyMoment) {
        ...
        button.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            this.getEntry('activeKeyMoment').value = index;
        }));
    ...
    update(compareResult) {
        ...
        if(changedMap.has('keyMoments')) {
            ...
            for(const [i, keyMoment] of keyMoments) {
                const [li] = this._createKeyMomentButton(i, keyMoment);
                if(activeIndex === i)
    ...
    constructor(parentAPI, zones) {
        ...
        const myParentAPI = {
          ...
          , getEntry: (externalName) => {
                ...
                if(externalName.startsWith(this.parentAPI.rootPath.toString())) {
