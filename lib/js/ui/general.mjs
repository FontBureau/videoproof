...
export class UIControllerBase extends UIBase {
    ...
    update(compareResult) {
        const rootState = this.parentAPI.getEntry('/')
          , [changed, compareDetailsMap] = processCompareResult(this.dependencies, rootState, compareResult)
          ;

        console.log(`${this.constructor.name}.update changed:`, changed, 'compareDetailsMap', compareDetailsMap);
        for(const widgetWrapper of this._widgets) {
            if(widgetWrapper.widget instanceof UIControllerBase) {
...
export class MainUIController extends UIControllerBase {
          ...
          , [
                {
                  ...
                  , activationTest:(parentAPI)=>{
                        const activeState = parentAPI.getEntry('/activeState');
                        return activeState.WrappedType instanceof ExampleLayoutModel;
                    }
                }
              , []
    ...
    get dependencies() {
        // In other cases, We can inject this from the
        // parent controller, but here, as the root, this would have
        // to happen in the shell, which is undesired.
        const dependencies = new Set(super.dependencies);
        dependencies.add(Path.stringSanitize(`${this.parentAPI.rootPath}/activeState`));
        return dependencies;
    }
}
