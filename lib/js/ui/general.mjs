...
class WidgetWrapper {
    constructor(parentAPI, dependencyMappings
        ...
        this.dependencies = new Set(this.dependencyMapping.keys());
...
function processCompareResult(uiDependencies, state, compareResult) {
    // TODO: Document!
    // COMPARE_STATUSES:
    ...
    const {EQUALS, CHANGED, NEW, DELETED, MOVED, LIST_NEW_ORDER} = COMPARE_STATUSES
      ...
      , changedMap = new Map()
      , compareDetailsMap = new Map()
      ;
    // FIXME: I think I'm not fully satisfied with this dumbing dowm
    ...
    for(const [status, data, pathInstance] of  compareResult) {
        ...
        if(status === LIST_NEW_ORDER) {
            ...
            const entry = getEntry(state, path);
    ...
    return [changedMap, compareDetailsMap];
}

function _getInitialCompareResults(dependencies) {
    const compareResult = [];
    ...
    *activeWidgets() {
    ...
    get dependencies() {
        ...
        for(const widgetWrapper of this.activeWidgets()) {
            for(const path of widgetWrapper.dependencies)
    ...
    initialUpdate(rootState) {
        ...
        // A) will be done in processCompareResult, however it's not
        ...
        const compareResult = _getInitialCompareResults(this.dependencies);
        // const [changedMap, compareDetailsMap] = processCompareResult(this.dependencies, rootState, compareResult);
    ...
    _update(compareResult, requiresFullInitialUpdateSet, isInitialUpdate) {
        const rootState = this.parentAPI.getEntry('/')
          , [changedRootMap, compareDetailsMap] = processCompareResult(this.dependencies, rootState, compareResult)
          ;

        console.log(`${this.constructor.name}.update changedRootMap:`, changedRootMap, 'compareDetailsMap', compareDetailsMap);
        for(const widgetWrapper of this.activeWidgets()) {
            ...
            if(widgetWrapper.WidgetClass.prototype instanceof UIControllerBase) {
                ...
                if(requiresFullInitialUpdate)
                    ...
                    widgetWrapper.widget.update(compareResult);
                continue;
            }
            ...
            //        actually
            if(!isInitialUpdate && requiresFullInitialUpdate) {
                const compareResult = _getInitialCompareResults(widgetWrapper.dependencies)
                  , [changedRootMap /*, compareDetailsMap*/] = processCompareResult(widgetWrapper.dependencies, rootState, compareResult)
                  ;
                this._updateSimpleWidget(widgetWrapper, changedRootMap);
            }
            else
                this._updateSimpleWidget(widgetWrapper, changedRootMap);
        }
    ...
    // This means propably just that the widget expects a changeLocaldMap
    // not a more complex structure, but more complex structures will
    // likely be required at some point.
    _updateSimpleWidget(widgetWrapper, changedRootMap) {
        const changeLocaldMap = new Map();

        // FIXME: this remapping should be done in widgetWrapper
        //        as it is the owner of the dependencyMapping.
        for(const [path, target] of widgetWrapper.dependencyMapping) {
            ...
            if(changedRootMap.has(path))
                changeLocaldMap.set(target, changedRootMap.get(path));
        }
        if(changeLocaldMap.size)
            widgetWrapper.widget.update(changeLocaldMap);
    }
}
          ...
          , [
                {
                  ...
                  , activationTest:()=>{
                        const activeState = this.parentAPI.getEntry('/activeState');
