...
export class SimpleProof extends UIBase {
    ...
    update (changedMap) {
       console.log(`${this.constructor.name}.update(changedMap):`, changedMap);
        ...
        // const {axisLocations, autoOPSZ, ...} this.getEntries(changedMap, 'axisLocations', 'autoOPSZ')
        // would be interesting to also extract value if possible!
        const axisLocations = changedMap.has('axisLocations')
                        ? changedMap.get('axisLocations')
                        : this.getEntry('axisLocations')
       ...
       if(changedMap.has('font'))
        ...
        // in the hasOPSZ case
        let requireVariationsUpdate = false;
        if(changedMap.has('axisLocations'))
            // When axisLocations changed we always update.
            requireVariationsUpdate = true;
        else if(hasOPSZ) {
            // When axisLocations changed we always update.
            // => has opsz, axisLocations did not change
            if(changedMap.has('autoOPSZ') // always
                    // => autoOPSZ did not change, what's the value ...
                    || (autoOPSZ && changedMap.has('fontSize')) // => we'll use fontSize and it changed
                    || (!autoOPSZ && changedMap.has('font')) // => we'll use the default value and the font changed
                    )
                requireVariationsUpdate = true;
        }
        // else if the font doesn't have an opsz only update when axisLocations changed

        if(requireVariationsUpdate) {
            const variations = [];
            for(const [axisTag, value] of axisLocations)
                variations.push(`"${axisTag}" ${value.value}`);
            if(hasOPSZ && !axisLocations.has('opsz')) {
            ...
            console.log('update axisLocations', ...variations, '::', axisLocations);
            // opsz can never be auto!!
          ...
          , widget = {
              ...
              , setInstances(instances) {
                    ...
                    options.push(makeOption({
                            value: UIManualAxisLocations.CUSTOM_STYLE_VALUE
                         , disabled: '1'
                    }, '(custom value)'));
    ...
    *axesEntries() {
    ...
    update(changedMap) {
        ...
        try {
            ...
            this._updateStyleSelect(originalLocations, this._autoOPSZInput.checked);
        }
    }
    ...
    _updateStyleSelect(originalLocations, autoOPSZ) {
        ...
        if(requireUpdateStyleSelect && this._styleSelect)
            this._styleSelect.setValue(this._localAxisLocations, autoOPSZ);
    }
}
