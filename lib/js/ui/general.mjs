...
class WidgetWrapper {
    constructor(hostElement, parentAPI, dependencyMappings, WidgetClass, ...widgetArgs) {
        ...
        this.widget = this.create({
              ...
              , getEntryRaw: this.parentAPI.getEntry
              , getEntry: (internalName)=>{
                  const externalName = this.dependencyReverseMapping.has(internalName)
                        ...
                        : internalName
                        ;
                  return this.parentAPI.getEntry(externalName);
                }
              , withChangeState: this.parentAPI.withChangeState
        }, ...widgetArgs);
    }
    ...
    create(parentAPI, ...args) {
        return new this.WidgetClass(parentAPI, ...args);
    }
}
