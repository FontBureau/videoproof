...
class KeyMomentsTimeline extends UIBase {
    ...
    constructor(parentAPI) {
        super(parentAPI);
        [this.element, this.itemsContainer, this.addButton, this.removeButton] = this.initTemplate();
    }
    ...
    initTemplate() {
        ...
        addButton.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            ...
            if(activeKeyMoment.value !== ForeignKey.NULL) {
                ...
                const index = parseInt(activeKeyMoment.value, 10) + 1;
                ...
                // just insert at end
                const size = keyMoments.push(newEntry);
                // select new entry
...
class MoveItemInListButton extends UIBase {
    ...
    // jshint ignore:end
    constructor(parentAPI, action, listProperty, indexProperty) {
        super(parentAPI);
        ...
        this.element.addEventListener('click', this._changeStateHandler((/*event*/)=>{
            const list = this.getEntry(listProperty)
              , key =  this.getEntry(indexProperty)
              ;
    ...
    constructor(parentAPI, zones) {
        const widgets = [
          ...
          , [
                {zone: 'main'}
              , [
                    // no dependencies, as we don't read/update anything
                    // there are just writes on change.
                ]
              , MoveItemInListButton
              , MoveItemInListButton.BACKWARD // action
              , parentAPI.rootPath.append('../keyMoments')// list property
              , parentAPI.rootPath.append('../activeKeyMoment')// index property => NULL_ALLOWED
            ]
          , [
                {zone: 'main'}
              , [
