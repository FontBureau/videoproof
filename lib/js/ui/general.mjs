...
class UIManualAxisLocations extends UIBase {
    ...
    constructor (parentAPI) {
        ...
        this._autoOPSZChangeHandler = this._changeStateHandler(event=>{
            event.preventDefault();
            ...
            if(this._autoOPSZInput.checked) {
                ...
                if(defaultValue === value)
                    axisLocations.delete(axisTag);
                else {
                    // TODO: definitely need a shortcut
                    if(!axisLocations.has(axisTag))
                        axisLocations.set(axisTag, axisLocations.constructor.Model.createPrimalDraft());
                    axisLocations.get(axisTag).set(value);
                }
            }
        });

        this._axisChangeHandler = this._changeStateHandler((axisTag, value)=>{
            const axisLocations = this.getEntry('axisLocations');

            // TODO: set should work without having to create the model
            //       maybe another interface, like setValue when Model is a simple model
            //       with a set(value) method...
            //       Or even an automatism in that case!
            if(!axisLocations.has(axisTag))
                axisLocations.set(axisTag, axisLocations.constructor.Model.createPrimalDraft());
            axisLocations.get(axisTag).set(value);
        });

        this._styleSelectChangeHandler= this._changeStateHandler((locations)=>{
        ...
        this.element = this._domTool.createElement('div', {class: 'manual_axis_locatioms'},  this._domTool.createElement('h3', {}, 'Manual Axis Locations'));
