...
class TimeControlCircle extends UIBase {
    //jshint ignore:start
    ...
    _onPointerDownHandler(event) {
        ...
        this._removeEventListeners();
        this._windowEventListeners.push(['pointermove', this._onPointerMoveHandler.bind(this)]);
    ...
    _onPointerMoveHandler(event) {
        ...
        const clientRect = this.element.getBoundingClientRect()
         ...
         , t =  (Math.atan2(y, x) / Math.PI / 2 + 1 - this._rotateStart) % 1
         ;
        this._setT(t);
    ...
    getXYForT(t, radius, rotateStart=0, translate={x:0, y:0}) {
        return [
            Math.cos((rotateStart + t) * 2 * Math.PI) * radius + translate.x
          , Math.sin((rotateStart + t) * 2 * Math.PI) * radius + translate.y
        ];
    }
...
function* animationGenerator(performanceAPI, duration, perpetual, newT) {
    let t = 0
      , lastExecution
      ;
    // run forever
