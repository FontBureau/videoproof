...
function potentialWriteMethodProxy (fnName, fn, {hasDraft, getDraft, getPotentialWriteProxy}) {
    ...
    const handler = {
      ...
      , apply: function(targetFn, thisArgument, argumentsList) {
            ...
            const draftOrThis = hasDraft() ? getDraft() : thisArgument;
            let result;
            try {
...
class PotentialWriteProxy {
    constructor(parent, immutable, key=null) {
        ...
        this._metodProxyAPI = {
                hasDraft: this.hasDraft.bind(this)
              , getDraft: this.getDraft.bind(this)
              , getPotentialWriteProxy: this.getPotentialWriteProxy.bind(this)
        };
    ...
    _handlerGet (target, prop, receiver) {
        ...
        if(typeof result === 'function') {
            ...
            return potentialWriteMethodProxy(prop, result, this._metodProxyAPI);
        }
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    *[Symbol.iterator]() {
    ...
    getDraft(key, defaultReturn=_NOTDEF) {
...
export class _AbstractOrderedMapModel extends _BaseContainerModel {
    ...
    constructor(oldState=null, dependencies=null) {
        ...
        // Entries may get replaced by set or moved/removed by splice.
        this[_OLD_TO_NEW_SLOT] = this._value.map(
    ...
    metamorphose(dependencies={}) {
        ...
        for(const index of this._value.keys()) {
            ...
            if(!kvItem &&  this[OLD_STATE] !== null) {
                const [oldIndex, /*key, proxy*/] = this[_OLD_TO_NEW_SLOT].getIndex(index);
                kvItem = this[OLD_STATE].get(oldIndex);
            }
        ...
        if(this[OLD_STATE] !== null
                ...
                && this._value.every((entry, index)=>{
                        const [key, value] = entry
                          , [oldKey, oldValue] = this[OLD_STATE].get(index)
                          ;
    ...
    *[Symbol.iterator]() {
        for(const kv of this._value) yield kv;
    }
    ...
    *entries() {
    ...
    arraySplice(index, deleteCount, ...entries) {
        ...
        const _entries = entries.map(kv=>Object.freeze(kv.slice())) // defensive copy of the k,v pairs
          , deleted = this._value.splice(index, deleteCount, ..._entries)
          ;
        ...
        for(let i=this._value.length-1; i>=0;i--) {
            const kv = this._value[i]
              , [key, /*value*/] = this._value[i]
              ;
            if(seen.has(key)) {
                // remove duplicate
    ...
    // append, add to end
    push(...entries) {
        return this.arraySplice(Infinity, 0, ...entries);
    }
    // add to front
    unshift(...entries) {
        return this.arraySplice(0, 0, ...entries);
    }
    // remove from end
    pop() {
