...
const _IS_POTENTIAL_WRITE_PROXY = Symbol('_IS_POTENTIAL_WRITE_PROXY')
  , _POTENTIAL_WRITE_PROXY_GET_IMMUTABLE = Symbol('_POTENTIAL_WRITE_PROXY_GET_IMMUTABLE')
  , _POTENTIAL_WRITE_PROXY_GET_DRAFT = Symbol('_POTENTIAL_WRITE_PROXY_GET_DRAFT')
  , _POTENTIAL_WRITE_PROXY_GET = Symbol('_POTENTIAL_WRITE_PROXY_GET')
...
function potentialWriteMethodProxy (fnName, fn, getDraft, getPotentialWriteProxy) {
    ...
    const handler = {
      ...
      , apply: function(targetFn, thisArgument, argumentsList) {
            ...
            let result;
            try {
                result = Reflect.apply(targetFn, thisArgument, argumentsList);
            }
            catch(error) {
            ...
            if(!getterAPIs.has(fnName) || !requiresPotentialWriteProxy(result))
                return result;

            // It's a getter AND requiresPotentialWriteProxy
            // i.e. proxify the next level of children.
            //
            // getter implies requiresPotentialWriteProxy === true
            ...
            return getPotentialWriteProxy(key, result);
        }
    };
...
class PotentialWriteProxy {
    constructor(parent, immutable, key=null) {
        this.immutable = immutable;
        this.parent = parent;
        // if(key !== null)
        //       assert parent[_IS_POTENTIAL_WRITE_PROXY]
        //            && parent[_POTENTIAL_WRITE_PROXY_GET_IMMUTABLE].get(key) === immutable
        this.key = key;
        this.draft = null;
    ...
    getDraft() {
        ...
        let draft = false;
        if(this.parent[_IS_POTENTIAL_WRITE_PROXY])
    ...
    getPotentialWriteProxy (key, item) {
        ...
        return potentialWriteProxy(this.proxy, item, key);
    }
    _handlerGet (target, prop, receiver) {
        // assert target === immutable
        if (prop === _IS_POTENTIAL_WRITE_PROXY)
            return true;
        if (prop === _POTENTIAL_WRITE_PROXY_GET_IMMUTABLE)
            return this.immutable;
        if (prop === _POTENTIAL_WRITE_PROXY_GET_DRAFT)
            return this.draft || undefined;
        if (prop === _POTENTIAL_WRITE_PROXY_GET)
        ...
        if(typeof result === 'function') {
            ...
            return potentialWriteMethodProxy(prop, result, this.getDraft.bind(this), this.getPotentialWriteProxy.bind(this));
        }
...
function potentialWriteProxy(parent, immutable, key=null) {
    ...
    if(parent[_IS_POTENTIAL_WRITE_PROXY]) {
        if(immutable !== parent[_POTENTIAL_WRITE_PROXY_GET_IMMUTABLE].get(key))
        ...
        return new PotentialWriteProxy(parent, immutable, key);
    }
    // can call without the parent.hasDraftFor check
    // as it must get called from within parent in this case!
    if(parent.isDraft)
        return new PotentialWriteProxy(parent, immutable);
    ...
    *[Symbol.iterator]() {
    ...
    [_HAS_DRAFT_FOR_PROXY](proxy) {
        ...
        const key = this[_LOCAL_PROXIES].byProxy.get(proxy)
          ...
          , item = this.value.get(key)
          ;
        if(!item || !item.isDraft)
            return false;

        // no need to check item[_DRAFT_ORIGIN] as we died identify it
        // via this[_LOCAL_PROXIES.
        return true;
    }
    ...
    [_GET_DRAFT_FOR_OLD_STATE_KEY](key) {
        ...
        const item = this.value.has(key)
            ? this.value.get(key) // => assert item.isDraft
    ...
    _get(key) {
        ...
        let item = this.value.has(key) && this.value.get(key);
        ...
        const proxyOrDraft = potentialWriteProxy(this, item);
        if(proxyOrDraft[_IS_POTENTIAL_WRITE_PROXY]) {
...
export class _AbstractListModel extends _BaseContainerModel {
    ...
    constructor(oldState=null, dependencies=null) {
        ...
        // Start with an empty this.value for quick not-changed comparison.
        Object.defineProperty(this, 'value', {
        ...
        // shadow of this.value that is kept in sync with value!
        // Entries may get replaced by set or moved/removed by splice.
        this[_OLD_TO_NEW_SLOT] = [...this.value.keys()]
    ...
    metamorphose(dependencies={}) {
        ...
        const dependenciesAreEqual = this[OLD_STATE] !== null
                && _dependenciesAreEqual(this[OLD_STATE].dependencies, this.dependcies);
        ...
        for(const index of this.value.keys()) {
            let item = Object.hasOwn(this.value, index) && this.value[index];
            if(!item &&  this[OLD_STATE] !== null) {
            ...
            const immutable = item.isDraft
                    ...
                    : (!_dependenciesAreEqual(this.dependencies, item.dependcies)
                          ? item.getDraft().metamorphose(this.dependencies)
                          : item
                      )
              ;
            this.value[index] = immutable;
        }
        // last stop to detect a no-change
        if(this[OLD_STATE] !== null
                && dependenciesAreEqual
                && this.size === this[OLD_STATE].size
                && this.value.every((entry, index)=>entry === this[OLD_STATE].get(index)))
            return this[OLD_STATE];
    ...
    keys() {
        return [this.value.keys()].map(i=>i.toString(10));
    }
    ...
    [_HAS_DRAFT_FOR_PROXY](proxy) {
        ...
        // May not be a draft at this moment!
        // In that case it may also not yet be in this.value.
        const item = Object.hasOwn(this.value, index) && this.value[index];
        if(item && !item.isDraft)
            return false;
    ...
    [_GET_DRAFT_FOR_OLD_STATE_KEY](oldKey) {
        ...
        if(!this.isDraft)
            // required: so this can be turned into a draft on demand
            throw immutableWriteError(new Error(`IMMUTABLE WRITE ATTEMPT `
                +`${this}.getDraftFor(${oldKey}) is immutable, not a draft.`));
        ...
        if(item.isDraft)
            // We used to set and check _DRAFT_ORIGIN in here
            // where draft[_DRAFT_ORIGIN] = [this, oldIndex];
            // and item[_DRAFT_ORIGIN][1] === oldIndex)
            // However, since we already created the connection between
            // index and oldIndex, we found index via oldKey,
            // item belongs to oldIndex.
            return item;
        const draft = item.getDraft();
        this.value[index] = draft;
        return draft;
    }
    ...
    [_GET_DRAFT_FOR_PROXY](proxy) {
        ...
        if(!item) {
            ...
            // must be an item in this.value or the proxy is disconnected.
            item = this[OLD_STATE].get(oldIndex);
        }
        if(item.isDraft)
            // since we found it via proxy, item belongs to it.
            // assert this.value[index] === item
            return item;
        const draft = item.getDraft();
        this.value[index] = draft;
        return draft;
    ...
    keyToIndex(key) {
        ...
        if(index < 0 || index >= this.value.length)
           return [null, `KEY ERROR NOT FOUND key "${key}" is not an index (= ${index})`
                       + ` (index > 0 && index < ${this.value.length}.`];
        return [index, null];
    }
    ...
    get(key, defaultReturn=_NOTDEF) {
        ...
        // else: It is a draft already and the draft is at this.value[index];
        return proxyOrDraft;
    }
    ...
    set(key, entry) {
        if(!this.isDraft)
            throw new immutableWriteError(new Error(`NOT DRAFT ERROR: ${this} can't call set when not in draft phase.`));
        const [index, message] = this.keyToIndex(key);
        if(index === null)
            throw new Error(message);
        ...
        if(this.value[index] === entry)
            return;
        // It makes sense, either the item is immutable or it is a draft
        // if it is immutable, the proxy would not make it a draft in here
        // otherwise, it is already a draft.
        // We could at this point though accept proxies and peel it off
        // later, so it would still take draft form if changed in between.
    ...
    splice(start, deleteCount, ...entries) {
        if(!this.isDraft)
            ...
            throw new immutableWriteError(new Error(`NOT DRAFT ERROR: ${this} can't call splice when not in draft phase.`));

        const removed = this.value.splice(start, deleteCount, ...entries);
...
export class _AbstractOrderedMapModel extends _BaseContainerModel {
    ...
    constructor(entries) {
...

        {
            ...
            for(const [key, entry] of entries) {
                if(!(entry instanceof this.constructor.Model))
                    typeFails.push(`${key} [at ${index}] ("${entry}" typeof ${typeof entry})`);
