...
export class _AbstractListModel extends _BaseContainerModel {
    ...
    constructor(oldState=null, dependencies=null) {
        ...
        Object.defineProperty(this, 'value', {
            value: this[OLD_STATE] !== null ? [...this[OLD_STATE].value] : []
    ...
    metamorphose(dependencies={}) {
        ...
        const dependenciesAreEqual = _dependenciesAreEqual(this[OLD_STATE].dependencies, this.dependcies)
        // Here's a shortcut:
        if(this[OLD_STATE] !== null
                   // otherwise the children must get updated (if there are any ...
                && dependenciesAreEqual

                   // FIXME: how to figure this out???
                   //       => we copied all children in the constructor,
                   //          breaks this check.
                   // no new drafts since going into draft mode, i.e.
                   // no potential changes that need to be checked.
                   // && this.value.size === 0)
                   //
                   // We can however compare all items by identity,
                   // the size comparison is important for the every line
                   // to be correct even when OLD_STATE has more items
                && this.value.size === this[OLD_STATE].size
                && this.value.every((entry, i)=>entry === this[OLD_STATE][i])
            return this[OLD_STATE];

        for(const [i, entry] of this.value.keys()) {
            const entry = this.value[i] !== undefined
                    ? this.value[i]
                    : (this[OLD_STATE] !== null && this[OLD_STATE].has(i)
                            ? this[OLD_STATE].get(i)
                            : undefined
                      )
                    ;
            // this[OLD_STATE].get(i) should be available!
            // well, depends, this.size can be > this[OLD_STATE].size
            // i.e. this.value[10000] = newEntry would create 10k empty
            // slots in an empty array.
            // NOTE: that should not be an allowed interaction!
            // But empty slots are interesting! the immutable version can't
            // have these! They will fail in the general type check later.
            if(entry === undefined)
                continue;

            const immutable = entry.isDraft
                ? entry.metamorphose(this.dependencies);
                  // Not sure if we should check with _dependenciesAreEqual
                   // or just let entry check itself if it has to move forward.
                : (!_dependenciesAreEqual(this.dependencies, entry.dependcies)
                        ? entry.getDraft().metamorphose(this.dependencies)
                        : entry
                  )
                ;
            this.value[i] = immutable;
        }
        ...
        {
            const typeFails = [];
            for(const [i, entry] of this.value.entries()) {
    ...
    // TODO:
    // The whole protentialWriteProxy story needs to be tackled next
    // in here, especially so metamorphose can be finished.
    // the getters/setters (also splice etc.) story below will have to be
    // done as well.
    hasDraftFor
    getDraftFor
    // especially the tracking of a draft "ceated here":
    if(item[_DRAFT_ORIGIN]
                && item[_DRAFT_ORIGIN][0] === this
                && item[_DRAFT_ORIGIN][1] === key)
    // seems interesting when the actual key has changed/moved because of
    ...
    push(...entries) {
        return this.splice(this.length, 0, ...entries);
    }
    unshift(...entries) {
