...
function potentialWriteMethodProxy (name, fn, getDraft, potentialWriteProxy) {
    ...
    const handler = {
      ...
      , apply: function(target, thisArgument, argumentsList) {
            ...
            // However, the treatment of result below would allow so.
            let result;
            try {
...
function potentialWriteProxy(parent, key) {
    ...
    parent.method hasDraftFor(key) {
        ...
        if(this.value.has(key)) {
            const draft = this.value.get(key);
            if(draft[DRAFT_ORIGIN] && draft[DRAFT_ORIGIN][0] === this
                    && draft[DRAFT_ORIGIN][1] === key)
                return true;
            // will retutn false
        }
        return false;
    ...
    parent.method getDraftFor(key) {
        ...
        // create a draft()
        const draft = this[OLD_STATE].get(key).getDraft();
        ...
        // we have to create a separate draft, that is not in this.value
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    // static createPrimalState or instane.getDraft
    ...
    // state.
    //
    // For a child to only update dependencies, the canonical way is
    // probaly to do next = child.getDraft().metamorphose(dependencies);
    constructor(oldState) {
        super();
        // begin the first lifecycle as a draft!
        // if not oldState this will be a primal state.
        //      FIXME: this.value must get populatet with defaults for
        //             the primal state case (all in draft models).
    ...
    static createPrimalState(dependencies) {
        return new this.constructor(null).metamorphose(dependencies);
    }
    ...
    //
    // THINKING: could this be called without depenedencies or only with
    // changed dependencies? I see no harm, initially, a lack of dependencies
    // would be detected.
    ...
    metamorphose(dependencies={}) {
        if(!this.isDraft)
            throw new Error(`LIFCYCLE ERROR: ${this} must be in draft mode to metamorphose.`);
        ...
        {
            console.log(`${this} dependencies `, this.constructor.dependencies, 'dependenciesData:', dependenciesData);
        ...
        // simple cases for items:
        // But this leaves out the generation and processing of childDependencies!
        //

        if this.values.has(name)// (a draft-item):
            // it is possible that metamorphose returns the OLD_STATE
            // of that draft and that that item is the same as
            // this[OLD_STATE].get(name)
            immutable = this.values.get(name).metamorphose(childDependencies);
        else this[OLD_STATE] !== null
            if dependencies did not change:
                immutable = this[OLD_STATE].get(name);
            else
                // same result as above if dependencies did not change
                // i.e. metamorphose will return this[OLD_STATE].get(name)
                // the question is if we need a shortcut for the above or if
                // we can always call the below.
                immutable = this[OLD_STATE].get(name).getDraft().metamorphose(childDependencies);
        else // this[OLD_STATE] === null this is a primal state
        ...
        // NEXT: THIS:::s
        // It is more complex;  coherence functions can still change
        // these items, hence, they must only be made immutable if they are
        // used as a dependency OR eventually after all dependencies have
        // been processed.
        //
        // Further, it would be nice when items are not already in draft
        // when used in a coherence function, they are only set to draft
        // when the coherence function is actually trying to write to them.
        // This is an optimization, as passing the draft and returning it
        // unchanged from metamorphose is a basic operation of this method!
        //
        // The optimization could be implemented using javascript proxies!
        // Similarly, in draft mode, that same proxy based implementation
        // could be used when items are just being read, we don't know if
        // a write follows.
        // The not optimized version is to put the items in draft mode when
        // used in coherence functions and retreived from [OLD_STATE] or
        // when being read in draft moode and being return from [OLD_STATE]
        //
        // Quite a difference it makes when an item is used as a dependency
        // and is thus being converted to immutable. Afterwards trying to
        // write should make a noise (throw an Error) and fail. Complicated
        // cases may be handled by reordering of coherence functions,
        // changed implementation of such functions or re-modelling.
        //
        ...
        for(const name of this.constructor.initOrder) {
            ...
            //
            //
            // about the potentially editable proxy
            //
            // soo, I read a value from e.g. this, which is in draft mode,
            // but the value is not (yet) in draft:
            //
            // I return a proxy witht hat value, when that is attempted to
            // be written:
            //    * the proxy creates the draft
            //         -> the draft may already exist from a previous
            //            attempt to write to the proxy, in that case,
            //            we write to the existing draft.
            //    * and redirects the write to the draftt
            //    * it also registers the draft in the this.value, potentially and
            //      prevents the proxy from being returned again
            //    * there's a cascading effect to be considered, the proxy
            //      may return another read value, which itself is potentially
            //      being written. In that case the sub-proxy must write to
            //      the super proxy, i.e. turn it into a draft, yada yada...
            //      This is kind of tricky.
            //
            //    Thus, we need to trap reads and writes. And we have a
            //    deep write cascading effect!
            //    What reads and writes are needs to be defined for all
            //    elements/types the proxy stuff better is universal i.e.
            //    _BaseModel or maybe _BaseContainerModel should define this stuff!
            //
            //    We start from a draft, but further we need to handle
            //    immmutables.
            ...
            if(this.constructor.childrenExternalDependencies.has(name)) {
                ...
                if(this.value.has(name)) {
                    ...
                    // FIXME: are the dependencies still correct?
                    //        if NOT we need to do something!
                    //        this is still a draft, however, it needs to
                    //        recalculate it's dependencies, a lot like
                    //        running instance.constructor again.
                    // FIXME: this is too early, but we have to do it before
                    //        we give it away as a dependency. This is in
                    //        conflict with the coherence-functions, which
                    //        may change the values (or may read-only as
                    //        well).
                    let value = instance.metamorphose();

                    localScope.set(name, value);
                }
                // If there's no oldState: does not happpen, it is taken care
                // of in the constructor and the previouis case applies.
                // assert this.value.has(name);
                else {
                    // FIXME: as in the case above, if this has changed dependencies
                    //        it must be changed. Unlike in the case above,
                    //        the way to do so would likely be:
                    //        value = value.getDraft(childDependencies).metamorphose();
                    // FIXME: Depending on the coherence function, if it wants to write,
                    //        we may have to put this into draft mode, this would be the
                    //        same mechanism as in the getter in draft mode. (not implemented).
                    //        Also the same, once this was given away as a dependency
                    //        it MUST NOT get written anymore by a coherence function.
                    value = oldEntries.get(name);//.getDraft(childDependencies);
                    localScope.set(name, value);
                }
            }
            else if(this.coherenceFunctions.has(name)) {
                ...
                // FIXME not a fan of using result like this! It makes stuff complicated!
                localScope.set(name, result);
            }
            else if(this.constructor.foreignKeys.has(name)) {
                // TODO: I think should look a lot like what was done in
                // the constructor. the value definitely depends on the
                // target and if the target changed so may has this. I'd
                // suggest to keep the old original key if it still links
                // to the identical target. Also, mut run key constraint
                // before vhecking all of this.

                // Key must not change anymore after used as an dependency!
        ...
        // Haha, I think this is BS as external dependencies should
        // rather not change after going into draft mode. however, above
        // there's some operational uncertainty!
        Object.defineProperty(this, 'dependencies', {
            value: this.dependencies
          , writable: false
            // explicitly not configuable anymore!
          , configurable: false
        });
    ...
    *[Symbol.iterator]() {
    ...
    _get(key) {
        // FIXME: in Draft, this.has has to work without all keys present in this.value!
        // but has could look at the constructor data.
        if(!this.has(key))
            throw new Error(`KEY ERROR "${key}" not found in ${this}.`);
            // OLD:    return this.value.get(key);

        if(this.value.has(key))
            // In draft mode, the element for key is in draft/draft-ready mode as well
            return this.value.get(key);


        // FIXME: do we have a draft mode for the other items in value as
        // well (keys, links)
        // FIXME: draft-ready how would this look like.
        if(!this.isDraft)
            // This would just be weired!
            throw new Error(`"${key}" should exist, but it doesn't`);

        // If there's no OLD_STATE this.value is already fully populated
        // with primal state draft elements by the constructor.
        const element = this[OLD_STATE].get(key).getDraft();
        this.value.set(key, element);
        return element;
    }
    ...
    set(key, entry) {
        if(!this.isDraft)
            throw new Error(`NOT DRAFT ERROR: ${this} can't call set when not in draft phase.`)
