...
// generic helper in metamorphose
export function collectDependencies(dependencyNamesSet, updatedDependencies, oldStateDependencies=null) {
...
export class _AbstractStructModel extends _BaseContainerModel {
    ...
    metamorphose(dependencies={}) {
        ...
        const localScope = new Map()
          ...
          , collectChildDependencies = dependencies=>Object.fromEntries(
          ...
          , lockItem = (name)=>{
                ...
                if(descriptor instanceof ForeignKey) {
                    ...
                    const childDependencies = collectChildDependencies(descriptor.dependencies);
        ...
        for(const name of this.constructor.initOrder) {
            ...
            if(this.constructor.childrenExternalDependencies.has(name)) {
                ...
                if(this[OLD_STATE] === null && !this._value.has(name)) {
                    ...
                    // for a new instance.
                    const childDependencies = collectChildDependencies(Model.dependencies);
                    ...
                    const immutable = Model.createPrimalState(childDependencies);
                ...
                const coherenceFunction = this.constructor.coherenceFunctions.get(name)
                  ...
                  , childDependencies = collectChildDependencies(coherenceFunction.dependencies)
                  ;
...
export class _AbstractListModel extends _BaseContainerModel {
    ...
    metamorphose(dependencies={}) {
        ...
        if(dependenciesAreEqual
                ...
                && Object.values(this.value).length === 0
        )
            return this[OLD_STATE];
...
export class _AbstractDynamicStructModel extends _BaseContainerModel {
    ...
    static createClass(className, BaseType /* a Base Model/Type */
        ...
        const result = {[className]: class extends this {
            ...
            static dependenciesNames = Object.freeze(new FreezableSet([modelDependencyName, ...dependenciesNames]));
            // jshint ignore: end
            }};
        // Can't override class.Model anymore, would be possible w/o the freeze.
        Object.freeze(result[className]);
    ...
    // but with **DYNAMIC** depenedencies
    // we can make sure maybe, that there's a fixes set of external
    ...
    metamorphose(dependencies={}) {
        ...
        if(dependenciesAreEqual // includes that there's an old state
            ...
            // Has NOT changed!
            return this[OLD_STATE];
        console.log('this.WrappedType', this.WrappedType, 'dependencies:', this.WrappedType.dependencies);
        const childDependencies = Object.fromEntries(
                iterMap(this.WrappedType.dependencies, (key)=>[key, this.dependencies[key]]));
        if(this._value === null // only in primal state creation
            ...
            if(this._value)
                console.warn(`${this} overriding wrapped value from ${this._value} `
                          + `to WrappedType ${this.WrappedType.name}. `
                          + `Is a draft: ${this._value.isDraft} `);
            console.log('this.WrappedType', this.WrappedType);
            Object.defineProperty(this, '_value', {
                value: this.WrappedType.createPrimalState(childDependencies)
              , configureable: true
            });
        }
        else
            this._value = this._value.isDraft
                    ? this._value.metamorphose(childDependencies)
                    : (!objectEntriesAreEqual(childDependencies, this._value.dependencies)
                                ? this._value.getDraft().metamorphose(childDependencies)
                                : this._value
                      )
                    ;
    ...
    set wrapped(state) {
        ...
        if(!(state instanceof this.constructor.BaseType))
            new Error(`TYPE ERROR ${this} expects an instance of `
        ...
        if(state.constructor !== this.WrappedType)
            new Error(`TYPE ERROR ${this} expects a direct instance of`
                + `"${this.WrappedType.name}" but state item is "${state}".`);
    ...
    // As such, when this becomes a draft, wrapped could become a draft
    // as well ... ???

    // There's still an issue that needs debugging!
    // [_HAS_DRAFT_FOR_PROXY](proxy) {
    //      return this.wrapped[_HAS_DRAFT_FOR_PROXY](proxy);
    // }
    // [_HAS_DRAFT_FOR_OLD_STATE_KEY](key) {
    //     return this.wrapped[_HAS_DRAFT_FOR_OLD_STATE_KEY](key);
    // }
    // [_GET_DRAFT_FOR_OLD_STATE_KEY](key) {
    //     return this.wrapped[_GET_DRAFT_FOR_OLD_STATE_KEY](key);
    // }
    // [_GET_DRAFT_FOR_PROXY](proxy) {
    //     return this.wrapped[_GET_DRAFT_FOR_PROXY](proxy);
    // }
    get value() {
...
export class Path {
    ...
    toString(defaultAnchoring=null /*ROOT || RELATIVE || null */) {
        ...
        return (anchoring === null
                        ? this.parts
                        : [anchoring, ...this.parts]
